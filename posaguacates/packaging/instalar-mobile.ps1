...
if(-not(Get-NetFirewallRule -DisplayName 'POS Aguacates - Red local' -ErrorAction SilentlyContinue)){New-NetFirewallRule -DisplayName 'POS Aguacates - Red local' -Direction Inbound -Protocol TCP -LocalPort 3000 -Action Allow -Profile Private|Out-Null}
$saludable=$false;for($i=1;$i-le 30;$i++){Start-Sleep 2;try{$h=Invoke-RestMethod 'http://127.0.0.1:3000/health' -TimeoutSec 2;if($h.ok){$saludable=$true;break}}catch{}};if(-not $saludable){throw 'El servicio no respondió en /health.'}
