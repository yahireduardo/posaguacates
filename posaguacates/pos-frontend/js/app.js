...
document.getElementById('abrirPago')?.addEventListener('click',()=>{limpiarPagoCliente();modalPago.classList.remove('hidden');pagoFecha.value=new Date().toISOString().slice(0,10);buscarClientePago.focus()});
...
cargarPredicciones?.addEventListener('click',async()=>{try{const r=await api('/prediccion');prediccionesLista.innerHTML=filas(r.predicciones,p=>{const metricas=p.comparacion?.metricas||{},actual=metricas.PROMEDIO_PONDERADO_LOCAL,ganador=metricas[p.ganador];return `<div class="prediccion-producto"><div class="row"><span>${esc(p.nombre)} (${esc(p.unidad)})</span><strong>${cantidad(p.estimado_proxima_semana)} ± ${cantidad(p.incertidumbre)}</strong></div><small>${esc(p.observacion)}</small>${actual&&ganador?`<small>MAE actual: ${cantidad(actual.mae)} · MAE ganador: ${cantidad(ganador.mae)} · RMSE ganador: ${cantidad(ganador.rmse)}</small>`:''}</div>`;})+`${r.advertencia_ml?`<p class="error">${esc(r.advertencia_ml)}</p>`:''}<p class="muted">${esc(r.criterio_ganador)} ${esc(r.limitaciones)}</p>`;}catch(x){alert(x.message);}});
