...
function pintarHistorialPagosProveedor(proveedorId=0){
  ...
  historialPagosProveedores.innerHTML=filas(pagos,p=>`<div class="row"><span>${new Date(p.fecha).toLocaleString('es-MX')} · ${esc(p.proveedor)} · ${esc(p.metodo_pago)}${p.referencia?` · ${esc(p.referencia)}`:''}</span><strong>${dinero.format(p.monto)}</strong></div>`,'No hay pagos registrados para este proveedor.');
}
