const API=''; let usuario=null, token=localStorage.getItem('tokenPOS')||sessionStorage.getItem('tokenPOS'), productos=[], clientes=[], carrito=[], ultimaVenta=null, detalleVentaActual=null, grafica=null, graficaProductos=null, graficaClientes=null, graficaProveedores=null, productoSeleccionado = null, indiceResultadoActivo = -1, ordenCargada=null, reporteProductosActual=[], estadoInstanciaPOS=null, archivoRespaldoSeleccionado=null, analisisRespaldoActual=null, backupIdDescargado=null, reinicioRespaldoPendiente=false, idempotenciaVentaPendiente=null, idempotenciaMovimientoPendiente=null;
const fechaLocalISO=(fecha=new Date())=>`${fecha.getFullYear()}-${String(fecha.getMonth()+1).padStart(2,'0')}-${String(fecha.getDate()).padStart(2,'0')}`;
function prepararAccesibilidadBasica(){
  document.querySelectorAll('input:not([type="hidden"]),select,textarea').forEach(campo=>{
    if(campo.getAttribute('aria-label')||campo.getAttribute('aria-labelledby')||campo.closest('label')||document.querySelector(`label[for="${CSS.escape(campo.id||'__sin_id__')}"]`))return;
    const nombre=campo.getAttribute('placeholder')||campo.getAttribute('title')||campo.id;
    if(nombre)campo.setAttribute('aria-label',nombre);
  });
  document.querySelectorAll('.modal').forEach(modal=>{
    modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');
    const titulo=modal.querySelector('h1,h2,h3');if(titulo){if(!titulo.id)titulo.id=`${modal.id||'modal'}-titulo`;modal.setAttribute('aria-labelledby',titulo.id)}
  });
  document.querySelectorAll('button').forEach(boton=>{if(!boton.getAttribute('aria-label')&&!boton.textContent.trim()&&boton.title)boton.setAttribute('aria-label',boton.title)});
}
prepararAccesibilidadBasica();
...
document.getElementById('detalleVenta')?.addEventListener('click', evento => {
  ...
  if (boton) quitar(Number(boton.dataset.indice));
    ...
    venderBtn?.addEventListener('click', async () => {
      ...
      } catch (error) { if(!error.mostradoEnCampo)alert(error.message); }
    });
    ...
    document.getElementById('movimientoForm')?.addEventListener('submit',async e=>{e.preventDefault();const form=e.currentTarget,boton=form.querySelector('button[type="submit"],button:not([type])'),productoId=Number(productoMovimiento.value),valor=Number(document.getElementById('cantidadMovimiento').value),producto=productos.find(p=>p.id===productoId);if(!producto)return alert('Busca y selecciona un producto');if(!esCantidadValida(valor,producto.unidad))return alert(mensajeCantidad(producto.unidad));if(!idempotenciaMovimientoPendiente)idempotenciaMovimientoPendiente=crypto.randomUUID();if(boton)boton.disabled=true;try{await api('/inventario/movimiento',{method:'POST',headers:{'Idempotency-Key':idempotenciaMovimientoPendiente},body:JSON.stringify({producto_id:productoId,tipo:document.getElementById('tipoMovimiento').value,cantidad:valor,motivo:document.getElementById('motivoMovimiento').value,idempotency_key:idempotenciaMovimientoPendiente})});idempotenciaMovimientoPendiente=null;form.reset();resultadosProductoMovimiento.classList.add('hidden');productoMovimientoBuscar.focus();await cargarInventario()}catch(error){alert(error.message)}finally{if(boton)boton.disabled=false}});
...
function limpiarPagoCliente(){formPago.reset();formPago.classList.add('hidden');pagoClienteId.value='';pagoClienteNombre.textContent='';notasPago.innerHTML='';buscarClientePago.value='';resultadosClientePago.innerHTML='';gestorPagoCliente.reiniciar(0)}
document.getElementById('abrirPago')?.addEventListener('click',()=>{limpiarPagoCliente();modalPago.classList.remove('hidden');pagoFecha.value=fechaLocalISO();buscarClientePago.focus()});
document.getElementById('cerrarPago')?.addEventListener('click',()=>{modalPago.classList.add('hidden');limpiarPagoCliente()});
async function abrirPagoParaCliente(id,nombre=''){
  limpiarPagoCliente();
  modalPago.classList.remove('hidden');
  pagoFecha.value=fechaLocalISO();
...
async function adminConfiguracion(){const c=await api('/configuracion');configNombre.value=c.nombre_comercial||'';configRazon.value=c.razon_social||'';configDireccion.value=c.direccion||'';configTelefono.value=c.telefono||'';configRfc.value=c.rfc||'';configMensaje.value=c.mensaje_ticket||'';configPapel.value=c.papel_mm||80;configTicketEscala.value=c.ticket_escala_texto||100;configTicketMargen.value=c.ticket_margen_mm??2;configTicketMostrarLogo.checked=c.ticket_mostrar_logo!==0;configTicketLogoContraste.checked=c.ticket_logo_alto_contraste===1;configTicketLogoAncho.value=c.ticket_logo_ancho_pct||50;configStockMinimo.value=c.stock_minimo_default||0;configVencimiento.value=c.vencimiento_dias??30;configPolitica.value=c.politica_credito||'';}
...
function configurarCamposCantidadProducto(){productoMinimo.step=esCaja(productoUnidad.value)?'0.5':'0.01';productoMinimo.min='0'}
function stockMinimoValido(valor,unidad){const n=Number(valor);return Number.isFinite(n)&&n>=0&&(!esCaja(unidad)||Number.isInteger(n*2))}
productoAdminForm?.addEventListener('submit',async e=>{e.preventDefault();const id=productoAdminId.value,b={codigo:productoCodigo.value,nombre:productoNombre.value,descripcion:productoDescripcion.value,precio_venta:productoPrecio.value,costo:productoCosto.value,unidad:productoUnidad.value,stock_minimo:productoMinimo.value,proveedor_id:productoProveedor.value||null};if(!Number.isInteger(Number(b.precio_venta))||!Number.isInteger(Number(b.costo)))return alert('El precio de venta y el costo de compra deben capturarse en pesos sin centavos');if(!stockMinimoValido(b.stock_minimo,b.unidad))return alert(esCaja(b.unidad)?'El stock mínimo por caja debe avanzar de 0.5 en 0.5.':'El stock mínimo en kilos debe ser un número no negativo.');try{const r=await api(id?`/productos/${id}`:'/productos',{method:id?'PUT':'POST',body:JSON.stringify(b)}),productoId=Number(id||r.id),principal=Number(productoProveedor.value||0);await api(`/productos/${productoId}/proveedores`,{method:'PUT',body:JSON.stringify({proveedor_principal_id:principal||null,proveedores:principal?[{proveedor_id:principal}]:[]})});e.target.reset();productoAdminId.value='';configurarCamposCantidadProducto();await adminProductos();}catch(x){alert(x.message);}});
productosAdminLista?.addEventListener('click',async e=>{const id=Number(e.target.dataset.pe||e.target.dataset.ps||e.target.dataset.pd),p=productosAdmin.find(x=>x.id===id);try{if(e.target.dataset.pe){Object.assign(productoAdminId,{value:p.id});productoCodigo.value=p.codigo;productoNombre.value=p.nombre;productoDescripcion.value=p.descripcion||'';productoPrecio.value=p.precio_venta;productoCosto.value=p.costo;productoUnidad.value=String(p.unidad).toUpperCase();configurarCamposCantidadProducto();productoMinimo.value=p.stock_minimo;productoProveedor.value=p.proveedor_id||'';}if(e.target.dataset.ps){await api(`/productos/${id}/estado`,{method:'PATCH',body:JSON.stringify({activo:e.target.dataset.on==='1'})});await adminProductos();}if(e.target.dataset.pd){eliminarProductoId.value=String(id);textoEliminarProducto.textContent=`¿Deseas eliminar definitivamente ${p.codigo} · ${p.nombre}?`;modalEliminarProducto.classList.remove('hidden');}}catch(x){alert(x.message);}});
function cerrarModalEliminarProducto(){modalEliminarProducto.classList.add('hidden');eliminarProductoId.value=''}
cerrarEliminarProducto?.addEventListener('click',cerrarModalEliminarProducto);
cancelarEliminarProducto?.addEventListener('click',cerrarModalEliminarProducto);
modalEliminarProducto?.addEventListener('click',e=>{if(e.target===modalEliminarProducto)cerrarModalEliminarProducto()});
confirmarEliminarProducto?.addEventListener('click',async()=>{const id=Number(eliminarProductoId.value);if(!Number.isInteger(id)||id<=0)return;confirmarEliminarProducto.disabled=true;try{const r=await api(`/productos/${id}`,{method:'DELETE',body:JSON.stringify({confirmacion:'ELIMINAR PRODUCTO'})});alert(r.mensaje);cerrarModalEliminarProducto();await Promise.all([adminProductos(),cargarProductos(),cargarInventario()])}catch(x){alert(x.message)}finally{confirmarEliminarProducto.disabled=false}});
...
proveedoresLista?.addEventListener('click',async e=>{
  const id=Number(e.target.dataset.re||e.target.dataset.rs||e.target.dataset.rv||e.target.dataset.rd),p=proveedoresAdmin.find(x=>x.id===id);
  try{
    ...
    if(e.target.dataset.rd){eliminarProveedorId.value=String(id);textoEliminarProveedor.textContent=`¿Deseas eliminar definitivamente el proveedor ${p.nombre}?`;modalEliminarProveedor.classList.remove('hidden')}
...
function cerrarModalEliminarProveedor(){modalEliminarProveedor.classList.add('hidden');eliminarProveedorId.value=''}
cerrarEliminarProveedor?.addEventListener('click',cerrarModalEliminarProveedor);
cancelarEliminarProveedor?.addEventListener('click',cerrarModalEliminarProveedor);
modalEliminarProveedor?.addEventListener('click',e=>{if(e.target===modalEliminarProveedor)cerrarModalEliminarProveedor()});
confirmarEliminarProveedor?.addEventListener('click',async()=>{const id=Number(eliminarProveedorId.value);if(!Number.isInteger(id)||id<=0)return;confirmarEliminarProveedor.disabled=true;try{const r=await api(`/proveedores/${id}`,{method:'DELETE',body:JSON.stringify({confirmacion:'ELIMINAR PROVEEDOR'})});alert(r.mensaje);cerrarModalEliminarProveedor();proveedorDetalle.classList.add('hidden');await Promise.all([adminProveedores(),adminProductos()])}catch(x){alert(x.message)}finally{confirmarEliminarProveedor.disabled=false}});
...
configuracionForm?.addEventListener('submit',async e=>{e.preventDefault();try{await api('/configuracion',{method:'PUT',body:JSON.stringify({nombre_comercial:configNombre.value,razon_social:configRazon.value,direccion:configDireccion.value,telefono:configTelefono.value,rfc:configRfc.value,mensaje_ticket:configMensaje.value,moneda:'MXN',papel_mm:Number(configPapel.value),ticket_escala_texto:Number(configTicketEscala.value),ticket_margen_mm:Number(configTicketMargen.value),ticket_mostrar_logo:configTicketMostrarLogo.checked,ticket_logo_alto_contraste:configTicketLogoContraste.checked,ticket_logo_ancho_pct:Number(configTicketLogoAncho.value),stock_minimo_default:Number(configStockMinimo.value),vencimiento_dias:Number(configVencimiento.value),politica_credito:configPolitica.value})});alert('Configuración guardada');}catch(x){alert(x.message);}});
async function cargarReporteComprasProveedor(){
  ...
  if(!fin.value)fin.value=fechaLocalISO();
