...
proveedoresLista?.addEventListener('click',async e=>{
  ...
  try{
    ...
    if(e.target.dataset.rv){
      const d=await api(`/proveedores/${id}`);proveedorDetalle.classList.remove('hidden');
      const estadoCuenta=d.movimientos?.length?`<div class="tabla-contenedor"><table><thead><tr><th>Fecha</th><th>Movimiento</th><th>Folio</th><th>Cargo</th><th>Abono</th><th>Saldo</th><th>Detalle</th><th>Usuario</th></tr></thead><tbody>${d.movimientos.map(m=>`<tr><td>${new Date(m.fecha).toLocaleString('es-MX')}</td><td>${esc(m.tipo)} · ${esc(m.estado)}</td><td>${esc(m.id)} / ${esc(m.folio||'—')}</td><td>${dinero.format(m.cargo)}</td><td>${dinero.format(m.abono)}</td><td>${dinero.format(m.saldo)}</td><td>${esc(m.descripcion||'')}</td><td>${esc(m.usuario||'—')}</td></tr>`).join('')}</tbody></table></div>`:'<p>Sin movimientos de cuenta.</p>';
...
proveedorDetalle?.addEventListener('click',e=>{if(e.target.dataset.cerrarDetalle!==undefined)proveedorDetalle.classList.add('hidden')});
