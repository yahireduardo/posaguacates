...
usuariosLista?.addEventListener('click',async e=>{const id=Number(e.target.dataset.ue||e.target.dataset.us||e.target.dataset.up);try{if(e.target.dataset.ue){const u=usuariosLista._data.find(x=>x.id===id);usuarioAdminId.value=u.id;usuarioNombre.value=u.nombre;usuarioUsername.value=u.username;usuarioRol.value=u.rol;usuarioPassword.value='';usuarioPassword.required=false;}if(e.target.dataset.us){const activo=e.target.dataset.on==='1',motivo=activo?'Reactivación':prompt('Motivo');if(motivo){await api(`/usuarios/${id}/estado`,{method:'PATCH',body:JSON.stringify({activo,motivo})});await adminUsuarios();}}if(e.target.dataset.up){const password=prompt('Nueva contraseña (mínimo 8 caracteres)');if(password)await api(`/usuarios/${id}/password`,{method:'PUT',body:JSON.stringify({password})});}}catch(x){alert(x.message);}});
