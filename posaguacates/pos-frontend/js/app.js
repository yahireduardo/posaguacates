...
const formulariosEscritura=new Set(['editorOrden','clienteForm','formPago','movimientoForm','formPagoProveedor','formCancelarCompra']);
...
document.querySelectorAll('[data-section]').forEach(b=>b.addEventListener('click',async()=>{try{if(b.dataset.section==='productosAdmin')await Promise.all([adminProductos(),adminProveedores()]);if(b.dataset.section==='proveedores')await adminProveedores();if(b.dataset.section==='compras'){await Promise.all([adminProveedores(),adminCompras()]);actualizarPasoCompra()}if(b.dataset.section==='usuarios')await adminUsuarios();if(b.dataset.section==='configuracion')await adminConfiguracion();if(b.dataset.section==='reportesAdmin')reportesDescargas.innerHTML=['ventas','pagos','cartera','inventario','productos','compras','cancelaciones'].map(t=>`<button data-csv="${t}">Descargar ${t}.csv</button>`).join('');}catch(x){alert(x.message);}}));
...
proveedoresLista?.addEventListener('click',async e=>{const id=Number(e.target.dataset.re||e.target.dataset.rs||e.target.dataset.rv),p=proveedoresAdmin.find(x=>x.id===id);try{if(e.target.dataset.re)abrirFormularioProveedor(p);if(e.target.dataset.rs){await api(`/proveedores/${id}/estado`,{method:'PATCH',body:JSON.stringify({activo:e.target.dataset.on==='1'})});await adminProveedores()}if(e.target.dataset.rv){const d=await api(`/proveedores/${id}`);proveedorDetalle.classList.remove('hidden');proveedorDetalle.innerHTML=`<div class="inventario-encabezado"><h3>${esc(d.proveedor.nombre)}</h3><button type="button" data-cerrar-detalle>Cerrar</button></div><div class="detalle-resumen"><div><span>Total comprado</span><strong>${dinero.format(d.proveedor.total_comprado)}</strong></div><div><span>Última compra</span><strong>${d.proveedor.ultima_compra?new Date(d.proveedor.ultima_compra).toLocaleDateString('es-MX'):'Sin compras'}</strong></div><div><span>Contacto</span><strong>${esc(d.proveedor.contacto||'—')}</strong></div><div><span>Estado</span><strong>${d.proveedor.activo?'Activo':'Inactivo'}</strong></div></div><p>${esc(d.proveedor.notas||'Sin notas')}</p><h4>Productos suministrados</h4>${filas(d.productos,x=>`<div class="row"><span>${esc(x.codigo)} · ${esc(x.nombre)} ${x.principal?'(principal)':''}</span><strong>${x.costo_ultimo==null?'—':dinero.format(x.costo_ultimo)}</strong></div>`)}<h4>Compras relacionadas</h4>${filas(d.compras,x=>`<div class="row"><span>${esc(x.folio)} · ${new Date(x.fecha).toLocaleDateString('es-MX')}</span><strong>${dinero.format(x.total)} · ${esc(x.estado)}</strong></div>`)}`;proveedorDetalle.scrollIntoView({behavior:'smooth'})}}catch(x){proveedorMensaje.textContent=x.message;proveedorMensaje.classList.remove('hidden')}});proveedorDetalle?.addEventListener('click',e=>{if(e.target.dataset.cerrarDetalle!==undefined)proveedorDetalle.classList.add('hidden')});
async function cargarPagosProveedores(){const d=await api('/cuentas-proveedores');resumenDeudaProveedores.innerHTML=`<div class="detalle-resumen"><div><span>Deuda total a proveedores</span><strong>${dinero.format(d.deuda_total)}</strong></div><div><span>Compras pendientes</span><strong>${d.cuentas.filter(c=>c.estado==='PENDIENTE').length}</strong></div></div>`;const pendientes=d.cuentas.filter(c=>c.estado==='PENDIENTE'&&Number(c.saldo_pendiente)>0);cuentaPagoProveedor.innerHTML=pendientes.length?'<option value="">Selecciona una compra</option>'+pendientes.map(c=>`<option value="${c.id}" data-saldo="${c.saldo_pendiente}">${esc(c.proveedor)} · ${esc(c.folio)} · saldo ${dinero.format(c.saldo_pendiente)}</option>`).join(''):'<option value="">No hay compras pendientes</option>';historialPagosProveedores.innerHTML=filas(d.pagos,p=>`<div class="row"><span>${new Date(p.fecha).toLocaleString('es-MX')} · ${esc(p.proveedor)} · ${esc(p.metodo_pago)}${p.referencia?` · ${esc(p.referencia)}`:''}</span><strong>${dinero.format(p.monto)}</strong></div>`);confirmarPagoProveedor.disabled=!pendientes.length}
...
compraForm?.addEventListener('submit',async e=>{e.preventDefault();if(!compraNueva.length)return alert('Agrega productos');const boton=confirmarCompra;if(boton.disabled)return;const k=crypto.randomUUID();boton.disabled=true;try{await api('/compras',{method:'POST',headers:{'Idempotency-Key':k},body:JSON.stringify({proveedor_id:Number(compraProveedor.value),folio:compraFolio.value,referencia:compraReferencia.value,observaciones:compraObservaciones.value,idempotency_key:k,productos:compraNueva.map(x=>({...x,unidad:productos.find(p=>p.id===x.producto_id)?.unidad}))})});compraNueva=[];e.target.reset();compraDetalle.innerHTML='';await Promise.all([adminCompras(),cargarInventario()])}catch(x){alert(x.message)}finally{boton.disabled=false}});
...
comprasLista?.addEventListener('click',e=>{if(!e.target.dataset.cc)return;cancelarCompraId.value=e.target.dataset.cc;modalCancelarCompra.classList.remove('hidden');setTimeout(()=>motivoCancelarCompra.focus(),0)});
