...
async function adminCompras(){const d=await api('/compras');comprasLista.innerHTML=filas(d,c=>`<div class="row"><span>${esc(c.folio)} · ${esc(c.proveedor||'')} · ${dinero.format(c.total)} · ${esc(c.estado)}<br><small>${c.estado_cuenta==='PENDIENTE'?`Por pagar: ${dinero.format(c.saldo_pendiente)}`:c.estado_cuenta==='PAGADA'?'Pagada':c.estado_cuenta==='CANCELADA'?'Deuda cancelada':'Sin cuenta'}</small></span><span>${c.estado==='ACTIVA'?`<button data-ce="${c.id}">Editar</button> <button data-cc="${c.id}" class="danger">Cancelar</button>`:''}</span></div>`);if(!productos.length)await cargarProductos();}
...
proveedoresLista?.addEventListener('click',async e=>{
  ...
  try{
    ...
    if(e.target.dataset.rv){
      ...
      const estadoCuenta=movimientos.length?`<div class="tabla-contenedor"><table><thead><tr><th>Fecha</th><th>Movimiento</th><th>Folio</th><th>Cargo</th><th>Abono</th><th>Saldo</th><th>Detalle</th><th>Usuario</th></tr></thead><tbody>${movimientos.map(m=>`<tr><td>${new Date(m.fecha).toLocaleString('es-MX')}</td><td>${esc(m.tipo)} · ${esc(m.estado)}</td><td>${esc(m.id)} / ${esc(m.folio||'—')}</td><td>${dinero.format(m.cargo)}</td><td>${dinero.format(m.abono)}</td><td>${dinero.format(m.saldo)}</td><td>${esc(m.descripcion||'')}</td><td>${esc(m.usuario||'—')}</td></tr>`).join('')}</tbody></table></div>`:'<p>Sin movimientos de cuenta.</p>';
