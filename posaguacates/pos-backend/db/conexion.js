...
async function esperarConexion(opciones = {}) {
  const intentos = Number(
    opciones.intentos || process.env.DB_CONNECT_RETRIES || 30,
  );
  const intervaloMs = Number(
    opciones.intervaloMs || process.env.DB_CONNECT_RETRY_MS || 2000,
  );
  for (let intento = 1; intento <= intentos; intento += 1) {
    try {
      ...
      console.error(
        `MySQL no disponible (intento ${intento}/${intentos}): ${error.code || error.message}`,
      );
