...
const upload = multer({
  ...
  fileFilter(req, file, callback) {
    ...
    if (![".zip", ".sql"].includes(ext)) {
      return callback(
        Object.assign(new Error("Solo se aceptan respaldos ZIP o SQL"), {
          status: 400,
        }),
      );
    }
...
async function confirmarPasswordAdmin(req) {
  ...
  if (!password)
    throw Object.assign(
      new Error("Se requiere la contraseña del administrador"),
      { status: 400 },
    );
  ...
  if (
    ...
    throw Object.assign(new Error("Autorización administrativa incorrecta"), {
      status: 401,
    });
  }
}
...
router.get("/status", async (req, res, next) => {
  try {
    res.json({
      ok: true,
      instance: await runtime.instanceControl.obtenerEstado(),
    });
  } catch (error) {
    next(error);
  }
...
router.post("/analyze", (req, res, next) => {
  upload.single("backup")(req, res, async (error) => {
    if (error)
      return next(
        Object.assign(error, {
          status: error.code === "LIMIT_FILE_SIZE" ? 413 : error.status || 400,
        }),
      );
    try {
      const resultado = await runtime.restoreService.analizar(
        req.file,
        req.usuario.id,
      );
...
router.post("/mark-transferred", async (req, res, next) => {
  try {
    ...
    if (!/^[0-9a-f-]{36}$/i.test(backupId))
      return res.status(400).json({ error: "backupId inválido" });
...
router.post("/reactivate", async (req, res, next) => {
  try {
    ...
    if (motivo.length < 10 || motivo.length > 500) {
      return res
        .status(400)
        .json({ error: "El motivo debe contener entre 10 y 500 caracteres" });
    }
    ...
    res.json({
      ok: true,
      estado: "ACTIVA",
      advertencia: "La reactivación manual puede causar divergencia de datos.",
    });
  } catch (error) {
    next(error);
  }
