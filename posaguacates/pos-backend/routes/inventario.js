...
router.get("/", async (req, res, next) => {
  ...
  try {
    const [rows] = await db.promise.query(
      ...
      [tipo, tipo, productoId, productoId],
    );
    res.json(rows);
  } catch (error) {
    next(error);
...
router.post("/movimiento", async (req, res) => {
  ...
  try {
    ...
    const [productos] = await connection.query(
      "SELECT stock, unidad FROM productos WHERE id = ? AND activo = 1 FOR UPDATE",
      [productoId],
    );
    if (!productos.length)
      throw Object.assign(new Error("Producto no encontrado"), { status: 404 });
    if (!esCantidadValida(cantidad, productos[0].unidad)) {
      throw Object.assign(new Error(mensajeCantidad(productos[0].unidad)), {
        status: 400,
      });
    }
    ...
    const nuevoStock =
      tipo === "AJUSTE"
        ? cantidad
        : stockAnterior + (tipo === "ENTRADA" ? cantidad : -cantidad);
    if (nuevoStock < 0)
    ...
    await connection.query(
      ...
      [
        productoId,
        tipo,
        tipo === "AJUSTE" ? Math.abs(nuevoStock - stockAnterior) : cantidad,
        stockAnterior,
        nuevoStock,
        motivo,
        String(req.body.referencia_tipo || "MANUAL").trim(),
    ...
    res
      .status(201)
      .json({ mensaje: "Movimiento registrado", stock: nuevoStock });
  } catch (error) {
