...
async function diagnosticoEliminacion(query, clienteId, bloquear = false) {
  ...
  return {
    ...
    modo: eliminacionDestructivaHabilitada(process.env)
      ? "DESTRUCTIVO_PRUEBAS"
      : "BAJA_LOGICA",
...
router.get("/", async (req, res) => {
  ...
  const limite = Math.min(
    500,
    Math.max(1, Number.parseInt(req.query.limite, 10) || 200),
  );
  const params = [];
  let where = "activo = 1";
  if (buscar) {
    where +=
      " AND (nombre_razon_social LIKE ? OR rfc LIKE ? OR telefono LIKE ?)";
  ...
  try {
    ...
    const [[total]] = await db.promise.query(
      `SELECT COUNT(*) total FROM clientes WHERE ${where}`,
      params,
    );
    res.json(
      req.query.pagina || req.query.buscar
        ? { datos: rows, pagina, limite, total: total.total }
        : rows,
    );
  } catch (error) {
    res.status(500).json({ error: "No fue posible consultar clientes" });
  }
});
...
router.get("/:id/resumen", async (req, res) => {
  ...
  try {
    const [[cliente], [cuentas], [movimientos], [ordenes], [ventas]] =
      await Promise.all([
        db.promise.query(
          `SELECT id,nombre_razon_social,rfc,telefono,direccion,correo_electronico,notas,permite_credito,activo FROM clientes WHERE id=?`,
          [id],
        ),
        db.promise.query(
          `SELECT id,venta_id,total_deuda,saldo_pendiente,estado,fecha
        FROM cuentas_por_cobrar WHERE cliente_id=? ORDER BY fecha,id`,
          [id],
        ),
        db.promise.query(
          `SELECT mc.*,u.nombre usuario FROM movimientos_cartera mc
        LEFT JOIN usuarios u ON u.id=mc.usuario_id
        WHERE mc.cliente_id=? ORDER BY DATE(mc.fecha) ASC,mc.id ASC`,
          [id],
        ),
        db.promise.query(
          `SELECT id,folio,estado,total_estimado,creada_at FROM ordenes_venta
        WHERE cliente_id=? AND estado='PENDIENTE' ORDER BY creada_at,id`,
          [id],
        ),
        db.promise.query(
          `SELECT id,total,tipo_pago,estado_pago,estado_venta,fecha FROM ventas
        WHERE cliente_id=? ORDER BY fecha DESC,id DESC LIMIT 20`,
          [id],
        ),
      ]);
    if (!cliente[0])
    ...
    res
      ...
      .json({
        error:
          "No fue posible consultar el estado de cuenta; verifica la migración pendiente",
      });
  }
});
...
function datosCliente(body) {
  return {
    ...
    correo:
      String(body.correo_electronico || "")
        .trim()
        .toLowerCase() || null,
    direccion: String(body.direccion || "").trim() || null,
    notas: String(body.notas || "").trim() || null,
    permiteCredito:
      body.permite_credito === false || body.permite_credito === 0 ? 0 : 1,
  };
}
...
async function crearCliente(req, res) {
  ...
  if (!c.nombre)
    return res.status(400).json({ error: "Nombre o razón social obligatorio" });
  try {
    ...
    res
      .status(201)
      .json({ mensaje: "Cliente creado", cliente_id: result.insertId });
  } catch (error) {
...
router.put("/:id", permitirRoles("ADMON_GRAL"), async (req, res) => {
  const id = Number(req.params.id);
  const c = datosCliente(req.body);
  if (!Number.isInteger(id) || id <= 0 || !c.nombre)
...
router.patch(
  ...
  async (req, res, next) => {
    const id = Number(req.params.id),
      activo = req.body.activo === true || req.body.activo === 1;
    if (!Number.isInteger(id) || id <= 0)
      return res.status(400).json({ error: "Cliente inválido" });
    try {
      const [[c]] = await db.promise.query(
        "SELECT nombre_razon_social FROM clientes WHERE id=?",
        [id],
      );
      if (!c) return res.status(404).json({ error: "Cliente no encontrado" });
      if (!activo && esPublicoGeneral({ id, nombre: c.nombre_razon_social }))
        return res
          .status(409)
          .json({ error: "Público General no se puede desactivar" });
      await db.promise.query("UPDATE clientes SET activo=? WHERE id=?", [
        activo ? 1 : 0,
        id,
      ]);
      res.json({
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "No fue posible analizar los datos del cliente" });
    }
  },
);
...
router.delete("/:id", permitirRoles("ADMON_GRAL"), async (req, res) => {
  ...
  const confirmacion = String(req.body.confirmacion || "")
    .trim()
    .toUpperCase();
  const errorValidacion = validarSolicitudEliminacion({
    clienteId,
    motivo,
    confirmacion,
  });
  if (errorValidacion)
    return res
      .status(errorValidacion.status)
      .json({ error: errorValidacion.error });
  ...
  try {
    ...
    if (!diagnostico) {
      const error = Object.assign(new Error("Cliente no encontrado"), {
        status: 404,
      });
      throw error;
    }
    if (diagnostico.protegido) {
      const error = Object.assign(
        new Error("El cliente Público General no se puede eliminar"),
        { status: 409 },
      );
      throw error;
    }
    ...
    if (!eliminacionDestructivaHabilitada(process.env)) {
      await connection.query("UPDATE clientes SET activo=0 WHERE id=?", [
        clienteId,
      ]);
      ...
      return res.json({
        ...
        modo: "BAJA_LOGICA",
        mensaje:
          "La eliminación destructiva está deshabilitada en producción. El cliente fue marcado como inactivo.",
        resumen: diagnostico,
      });
    }
    ...
    if (!Number(auditoria.existe)) {
      const error = Object.assign(
        new Error(
          "Falta aplicar la migración de auditoría; no se eliminó ningún dato",
        ),
        { status: 503 },
      );
      throw error;
    ...
    const ventasActivas = ventas
      .filter((v) => v.estado_venta === "ACTIVA")
      .map((v) => Number(v.id));
    ...
    if (ventasActivas.length) {
      ...
      for (const item of detalleActivo) {
        if (!esCantidadValida(item.cantidad, item.unidad)) {
          const error = Object.assign(
            new Error(
              `Cantidad inválida en la venta ${item.venta_id}: ${mensajeCantidad(item.unidad)}`,
            ),
            { status: 409 },
          );
          throw error;
    ...
    await connection.query(
      "DELETE FROM movimientos_cartera WHERE cliente_id=?",
      [clienteId],
    );
    if (pagoIds.length) {
      const marcadores = pagoIds.map(() => "?").join(",");
      await connection.query(
        `DELETE FROM aplicaciones_pago WHERE pago_id IN (${marcadores})`,
        pagoIds,
      );
      await connection.query(
        `DELETE FROM pagos WHERE id IN (${marcadores})`,
        pagoIds,
      );
    }
    if (cuentaIds.length) {
      ...
      await connection.query(
        `DELETE FROM aplicaciones_pago WHERE cuenta_id IN (${marcadores})`,
        cuentaIds,
      );
    }
    if (ordenIds.length) {
      ...
      await connection.query(
        `DELETE FROM detalle_orden_venta WHERE orden_id IN (${marcadores})`,
        ordenIds,
      );
      await connection.query(
        `DELETE FROM ordenes_venta WHERE id IN (${marcadores})`,
        ordenIds,
      );
    }
    await connection.query(
      "DELETE FROM cuentas_por_cobrar WHERE cliente_id=?",
      [clienteId],
    );
    if (ventaIds.length) {
      const marcadores = ventaIds.map(() => "?").join(",");
      await connection.query(
        `DELETE FROM detalle_venta WHERE venta_id IN (${marcadores})`,
        ventaIds,
      );
      await connection.query(
        `DELETE FROM ventas WHERE id IN (${marcadores})`,
        ventaIds,
      );
    }
    ...
    return res.status(error.status || 500).json({
      error: error.status
        ? error.message
        : "No fue posible eliminar el cliente; no se aplicaron cambios",
    });
  } finally {
    connection.release();
  }
