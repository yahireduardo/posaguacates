...
router.get("/compras-proveedores.csv", async (req, res, next) => {
  ...
  try {
    const [rows] = await db.promise.query(
        `SELECT p.nombre proveedor,COUNT(c.id) numero_compras,SUM(c.total) total_comprado,AVG(c.total) promedio_compra,MIN(c.fecha) primera_compra,MAX(c.fecha) ultima_compra FROM proveedores p JOIN compras c ON c.proveedor_id=p.id WHERE ${where.join(" AND ")} GROUP BY p.id,p.nombre ORDER BY total_comprado DESC,p.nombre`,
        params,
      ),
      columnas = [
        "proveedor",
      ...
      contenido = [columnas, ...rows.map((row) => columnas.map((c) => row[c]))]
        .map((row) => row.map(csvValue).join(","))
        .join("\r\n");
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="compras-por-proveedor.csv"',
    );
    res.send("\ufeff" + contenido);
  } catch (e) {
    next(e);
...
router.get("/:tipo.csv", async (req, res, next) => {
  const sql = reportesCsv[req.params.tipo];
  if (!sql) return res.status(404).json({ error: "Reporte no disponible" });
  try {
    const [rows] = await db.promise.query(sql),
      columnas = rows.length ? Object.keys(rows[0]) : [];
    const contenido = [
      columnas,
      ...rows.map((row) => columnas.map((c) => row[c])),
    ]
      .map((row) => row.map(csvValue).join(","))
      .join("\r\n");
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${req.params.tipo}.csv"`,
    );
    res.send("\ufeff" + contenido);
  } catch (e) {
    next(e);
