...
const csvValue=value=>{const texto=String(value??'');const seguro=/^[=+\-@\t\r]/.test(texto)?`'${texto}`:texto;return `"${seguro.replaceAll('"','""')}"`};
router.get('/compras-proveedores.csv',async(req,res,next)=>{const where=["c.estado='ACTIVA'"],params=[];if(req.query.fecha_inicio){where.push('c.fecha>=?');params.push(`${req.query.fecha_inicio} 00:00:00`)}if(req.query.fecha_fin){where.push('c.fecha<DATE_ADD(?,INTERVAL 1 DAY)');params.push(req.query.fecha_fin)}try{const[rows]=await db.promise.query(`SELECT p.nombre proveedor,COUNT(c.id) numero_compras,SUM(c.total) total_comprado,AVG(c.total) promedio_compra,MIN(c.fecha) primera_compra,MAX(c.fecha) ultima_compra FROM proveedores p JOIN compras c ON c.proveedor_id=p.id WHERE ${where.join(' AND ')} GROUP BY p.id,p.nombre ORDER BY total_comprado DESC,p.nombre`,params),columnas=['proveedor','numero_compras','total_comprado','promedio_compra','primera_compra','ultima_compra'],contenido=[columnas,...rows.map(row=>columnas.map(c=>row[c]))].map(row=>row.map(csvValue).join(',')).join('\r\n');res.setHeader('Content-Type','text/csv; charset=utf-8');res.setHeader('Content-Disposition','attachment; filename="compras-por-proveedor.csv"');res.send('\ufeff'+contenido)}catch(e){next(e)}});
