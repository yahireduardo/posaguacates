...
router.get("/:ventaId", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(401)
      .type("text")
      .send("El enlace de impresión es inválido o expiró");
  }
  if (
    !Number.isInteger(ventaId) ||
    ventaId <= 0 ||
    payload.proposito !== "TICKET" ||
    Number(payload.venta_id) !== ventaId
  ) {
    return res
      .status(403)
      .type("text")
      .send("El enlace no corresponde a este ticket");
  }
  ...
  try {
    ...
    if (venta.estado_venta === "CANCELADA") {
      ...
      return res
        .status(409)
        .type("text")
        .send("No se puede imprimir una venta cancelada");
    }
    ...
    const [formasPago] = await connection.query(
      `SELECT metodo_pago,monto,referencia FROM venta_formas_pago WHERE venta_id=? ORDER BY id`,
      [ventaId],
    );
    const totalRecibido = Number(
      formasPago.reduce((s, forma) => s + Number(forma.monto), 0).toFixed(2),
    );
    const cambio = formasPago.some((forma) => forma.metodo_pago === "EFECTIVO")
      ? Math.max(0, Number((totalRecibido - Number(venta.total)).toFixed(2)))
      : 0;
...
<tbody>${productos
      .map(
        (
          p,
        ) => `<tr><td>${esc(p.nombre)}<br><small>${esc(p.unidad || "")}</small></td>
<td>${esc(p.cantidad)}</td><td>${dinero.format(Number(p.precio_unitario))}</td>
<td>${dinero.format(Number(p.subtotal))}</td></tr>`,
      )
      .join("")}</tbody></table>
...
<script src="/js/ticket.js" defer></script></body></html>`);
    ...
    await connection.rollback();
    console.error("Error renderizando ticket:", {
      ventaId,
      code: error.code,
    ...
    return res
      .status(500)
      .type("text")
      .send("No fue posible generar el ticket");
  } finally {
    connection.release();
  }
});
