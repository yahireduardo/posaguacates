...
} = require("../lib/cancelacionPagoProveedor");
    ...
    const [cuentas, pagos] = await Promise.all([
      db.promise
        .query(
         ...
         ORDER BY cpp.estado='PENDIENTE' DESC,cpp.fecha,cpp.id`,
      ...
      db.promise
        .query(
         ...
         ORDER BY pp.fecha DESC,pp.id DESC LIMIT 300`,
          [proveedorId, proveedorId],
        )
        .then(([rows]) => rows),
    ]);
...
router.post("/pagos", async (req, res, next) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({
        error: "Cuenta, monto y método de pago válidos son obligatorios",
      });
  }
  if (metodo !== "EFECTIVO" && !referencia) {
    return res
      .status(400)
      .json({
        error: "La referencia es obligatoria para transferencia o cheque",
      });
  }
  ...
  try {
    ...
    if (cuenta.estado !== "PENDIENTE")
      throw fallo("La cuenta ya no está pendiente", 409);
    if (monto > Number(cuenta.saldo_pendiente))
      throw fallo("El pago no puede superar el saldo pendiente", 409);
    ...
    await connection.query(
      ...
      [
        req.usuario.id,
        String(pago.insertId),
        JSON.stringify({ cuenta_id: cuenta.id, monto, metodo_pago: metodo }),
      ],
    );
...
router.post("/pagos/:id/cancelar", async (req, res, next) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "Pago y motivo de 5 a 500 caracteres son obligatorios" });
  }
  const connection = await db.promise.getConnection();
  try {
    await connection.beginTransaction();
    const [[pago]] = await connection.query(
      "SELECT id,cuenta_id,proveedor_id,monto,estado FROM pagos_proveedores WHERE id=? FOR UPDATE",
      [id],
    );
    const [[cuenta]] = pago
      ? await connection.query(
          "SELECT id,total_deuda,saldo_pendiente,estado FROM cuentas_por_pagar_proveedores WHERE id=? FOR UPDATE",
          [pago.cuenta_id],
        )
      : [[null]];
    ...
    await connection.query(
      "UPDATE pagos_proveedores SET estado='CANCELADO' WHERE id=?",
      [id],
    );
    await connection.query(
      "UPDATE cuentas_por_pagar_proveedores SET saldo_pendiente=?,estado=? WHERE id=?",
    ...
    await connection.query(
      ...
      [
        ...
        JSON.stringify({
          ...
          saldo_restaurado: resultado.saldo_nuevo,
        }),
      ],
    );
    await connection.commit();
    res.json({
      mensaje: "Pago eliminado y saldo restaurado",
