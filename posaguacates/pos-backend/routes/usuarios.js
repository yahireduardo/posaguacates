...
const passwordValido = (value) =>
  typeof value === "string" && value.length >= 8 && value.length <= 128;
const fallo = (mensaje, status) =>
...
router.post("/", async (req, res, next) => {
  ...
  if (
    ...
    !["ADMON_GRAL", "CAJERO"].includes(rol)
  ) {
    return res
      .status(400)
      .json({
        error:
          "Nombre, username, contraseña (mínimo 8 caracteres) y rol válidos son obligatorios",
      });
  }
  try {
...
router.put("/:id", async (req, res, next) => {
  ...
  if (
    ...
    !usernameValido(username) ||
    !["ADMON_GRAL", "CAJERO"].includes(rol) ||
    (password && !passwordValido(password))
  ) {
  ...
  try {
    ...
    if (actual.rol === "ADMON_GRAL" && actual.activo && rol !== "ADMON_GRAL") {
      const [[admins]] = await connection.query(
        "SELECT COUNT(*) total FROM usuarios WHERE rol='ADMON_GRAL' AND activo=1 FOR UPDATE",
      );
    ...
    await auditar(connection, req.usuario.id, "EDITAR_USUARIO", id, null, {
      username,
      rol,
      password_actualizada: Boolean(password),
    });
...
router.patch("/:id/estado", async (req, res, next) => {
  ...
  if (!Number.isInteger(id) || id <= 0 || (!activo && !motivo))
    return res
      .status(400)
      .json({ error: "Usuario y motivo válidos son obligatorios" });
  if (id === Number(req.usuario.id) && !activo)
    return res
      .status(409)
      .json({ error: "No puedes desactivar tu propia sesión" });
  const connection = await db.promise.getConnection();
  try {
    await connection.beginTransaction();
    const [[actual]] = await connection.query(
      "SELECT rol,activo FROM usuarios WHERE id=? FOR UPDATE",
      [id],
    );
    if (!actual) throw fallo("Usuario no encontrado", 404);
    ...
    await connection.query("UPDATE usuarios SET activo=? WHERE id=?", [
      activo ? 1 : 0,
      id,
    ]);
    await auditar(
...
router.put("/:id/password", async (req, res, next) => {
  const id = Number(req.params.id);
  const password = String(req.body.password || "");
  if (!Number.isInteger(id) || id <= 0 || !passwordValido(password))
    return res
      .status(400)
      .json({ error: "La contraseña debe tener entre 8 y 128 caracteres" });
  try {
    const hash = await bcrypt.hash(password, 12);
    const [result] = await db.promise.query(
