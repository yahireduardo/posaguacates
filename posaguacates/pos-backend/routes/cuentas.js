...
const fallo = (message, status = 400) =>
...
router.get("/", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "No fue posible consultar cuentas" });
  }
});
  ...
  try {
    const [rows] = await db.promise.query(
      `SELECT c.id,c.nombre_razon_social,c.rfc,c.telefono,c.correo_electronico,
       ...
       ORDER BY c.nombre_razon_social LIMIT 50`,
      [q, q, q, q],
    );
    res.json(rows);
  } catch (e) {
    res.status(500).json({ error: "No fue posible buscar clientes" });
  }
});
...
async function pendientes(req, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "No fue posible consultar notas pendientes" });
  }
}
  ...
  try {
    ...
    const [cuentas] = await db.promise.query(
      `SELECT cxc.id,cxc.venta_id,cxc.total_deuda,cxc.saldo_pendiente,cxc.estado,cxc.fecha
       FROM cuentas_por_cobrar cxc WHERE cxc.cliente_id=? ORDER BY cxc.fecha,cxc.id`,
      [Number(req.params.id)],
    );
    const [pagos] = await db.promise.query(
      `SELECT p.id pago_id,ap.id aplicacion_id,ap.cuenta_id,ap.monto_aplicado monto,
       ...
       WHERE p.cliente_id=? ORDER BY p.fecha,p.id,ap.id`,
      [Number(req.params.id)],
    );
    ...
    res.json({
      ...
      movimientos,
      resumen: {
        saldo_total_pendiente: cuentas
          .filter((c) => c.estado === "PENDIENTE")
          .reduce((s, c) => s + Number(c.saldo_pendiente), 0),
        total_cuentas: cuentas.length,
        total_pagos: new Set(pagos.map((p) => p.pago_id)).size,
        ventas_pendientes: cuentas.filter((c) => c.estado === "PENDIENTE")
          .length,
        pagos_aplicados: new Set(
          pagos.filter((p) => p.pago_estado === "ACTIVO").map((p) => p.pago_id),
        ).size,
        pagos_cancelados: new Set(
          pagos
            .filter((p) => p.pago_estado === "CANCELADO")
            .map((p) => p.pago_id),
        ).size,
      },
    });
  } catch (e) {
    console.error("Error consultando cuenta de cliente:", {
    ...
    res
      .status(500)
      .json({ error: "No fue posible consultar el historial de la cuenta" });
  }
});
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!idValido(clienteId) || !Number.isFinite(monto) || monto <= 0)
      return res
        .status(400)
        .json({ error: "Cliente y monto válidos son obligatorios" });
    let desglose;
    try {
      desglose = normalizarMetodosPago(req.body, monto);
    } catch (error) {
      return res.status(error.status || 400).json({ error: error.message });
    }
    const metodo = desglose.metodo_resumen,
      referencia = desglose.referencia_resumen;
    const ids = [
      ...new Set((req.body.cuenta_ids || []).map(Number).filter(idValido)),
    ].sort((a, b) => a - b);
    if (!ids.length)
      return res.status(400).json({ error: "Selecciona al menos una nota" });
    ...
    try {
      ...
      const [cuentas] = await connection.query(
       ...
       FROM cuentas_por_cobrar WHERE id IN (${ids.map(() => "?").join(",")}) ORDER BY fecha,id FOR UPDATE`,
        ids,
      );
      if (cuentas.length !== ids.length) {
        throw fallo(
          "Una de las notas seleccionadas ya no existe. Actualiza la lista e intenta nuevamente",
          409,
        );
      }
      ...
      if (cuentas.some((c) => c.estado !== "PENDIENTE")) {
        throw fallo(
          "Una nota ya fue pagada o cancelada. Vuelve a seleccionar el cliente para actualizar saldos",
          409,
        );
      }
      ...
      for (const a of aplicaciones) {
        ...
        await connection.query(
         ...
         VALUES (?,?,?,?,?)`,
          [pago.insertId, a.cuenta.id, a.monto, anterior, nuevo],
        );
        await connection.query(
        ...
        await connection.query(
         ...
         VALUES (?,?,?,?,NOW(),'COBRO',?,0,?,?,?,?)`,
          [
            ...
            a.monto,
            saldo.total,
            String(req.body.observaciones || "").trim() || "Aplicación de pago",
            req.usuario.id,
          ],
        );
        recibo.push({
          cuenta_id: a.cuenta.id,
          venta_id: a.cuenta.venta_id,
      ...
      res
        .status(201)
        .json({
          pago_id: pago.insertId,
          cliente_id: clienteId,
          monto_total: monto,
          metodo_pago: metodo,
          metodos_pago: desglose.metodos,
          referencia,
          aplicaciones: recibo,
        });
    } catch (e) {
      ...
      res
        .status(e.status || 500)
        .json({
          error: e.status ? e.message : "No fue posible aplicar el pago",
        });
    } finally {
      connection.release();
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    if (motivo.length > 255)
      return res
        .status(400)
        .json({ error: "El motivo no puede exceder 255 caracteres" });
    let autorizacion;
    try {
    ...
    try {
      await connection.beginTransaction();
      const [[pago]] = await connection.query(
        `SELECT id,cliente_id,monto_total,estado FROM pagos WHERE id=? FOR UPDATE`,
        [pagoId],
      );
      const [aplicaciones] = await connection.query(
       ...
       WHERE ap.pago_id=? ORDER BY ap.id FOR UPDATE`,
        [pagoId],
      );
      ...
      await connection.query(
       ...
       WHERE id=?`,
        [autorizacion.autorizadoPor, motivo, pagoId],
      );
      for (const aplicacion of reversiones) {
        ...
        await connection.query(
          "UPDATE ventas SET estado_pago='PENDIENTE' WHERE id=?",
          [aplicacion.venta_id],
        );
        ...
        await connection.query(
         ...
         VALUES (?,?,?,NULL,NOW(),'CANCELACION_PAGO',?,?,0,?,?,?)`,
          [
            aplicacion.cliente_id,
            aplicacion.venta_id,
            aplicacion.cuenta_id,
            `CP-${pagoId}-${aplicacion.id}`,
            aplicacion.monto_aplicado,
            saldoCliente.total,
            `Cancelación del pago P-${pagoId}: ${motivo}`,
      ...
      res
        .status(e.status || 500)
        .json({
          error: e.status ? e.message : "No fue posible cancelar el pago",
        });
    } finally {
      connection.release();
    }
  ...
  try {
    const [[pago]] = await db.promise.query(
      `SELECT p.id,p.fecha,p.monto_total,p.metodo_pago,p.referencia,p.observaciones,p.estado,
       ...
       FROM pagos p JOIN clientes c ON c.id=p.cliente_id JOIN usuarios u ON u.id=p.usuario_id WHERE p.id=?`,
      [Number(req.params.id)],
    );
    if (!pago) return res.status(404).json({ error: "Pago no encontrado" });
    const [aplicaciones] = await db.promise.query(
      `SELECT ap.*,cxc.venta_id,CONCAT('V-',LPAD(cxc.venta_id,8,'0')) folio
       FROM aplicaciones_pago ap JOIN cuentas_por_cobrar cxc ON cxc.id=ap.cuenta_id WHERE ap.pago_id=? ORDER BY ap.id`,
      [pago.id],
    );
    const [formas_pago] = await db.promise.query(
      "SELECT metodo_pago,monto,referencia FROM pago_formas_pago WHERE pago_id=? ORDER BY id",
      [pago.id],
    );
    res.json({ pago, aplicaciones, formas_pago });
  } catch (e) {
    res.status(500).json({ error: "No fue posible generar el recibo" });
  }
});
