...
const {
  validarConfirmacionProveedor,
  proveedorTieneCompras,
} = require("../lib/eliminacionProveedor");
...
router.get("/", async (req, res, next) => {
  try {
    ...
    const incluirInactivos =
      req.usuario.rol === "ADMON_GRAL" &&
      ["TODOS", "INACTIVOS"].includes(estado);
    ...
    if (req.usuario.rol !== "ADMON_GRAL") {
      return res.json(
        rows.map(
          ({ total_comprado, deuda_pendiente, notas, direccion, ...row }) =>
            row,
        ),
      );
    }
  ...
  try {
    ...
    const [compras, productos, cuentas, pagos] = await Promise.all([
      ...
      db.promise
        .query(
         ...
         WHERE cpp.proveedor_id=? ORDER BY cpp.fecha,cpp.id`,
          [req.params.id],
        )
        .then(([rows]) => rows),
      db.promise
    ...
    const movimientos = [
      ...cuentas.map((c) => ({
        ...
        estado: c.estado,
        descripcion:
          c.estado === "CANCELADA" ? "Compra cancelada" : "Compra registrada",
      })),
      ...pagos.map((p) => ({
        id: `P-${p.id}`,
        fecha: p.fecha,
...
router.post("/", soloAdmin, async (req, res, next) => {
  const p = datosProveedor(req.body);
  const error = validarProveedor(p);
  if (error) return res.status(400).json({ error });
  try {
    if (await duplicado(db.promise, p))
      return res
        .status(409)
        .json({
          error: "Ya existe un proveedor con el mismo nombre, RFC o correo",
        });
    ...
    return res
      .status(201)
      .json({ id: result.insertId, mensaje: "Proveedor creado" });
  } catch (errorDb) {
    return next(errorDb);
  }
});
...
router.put("/:id", soloAdmin, async (req, res, next) => {
  const id = Number(req.params.id),
    p = datosProveedor(req.body),
    error = validarProveedor(p);
  if (!Number.isInteger(id) || id <= 0 || error)
    return res.status(400).json({ error: error || "Proveedor inválido" });
  try {
    if (await duplicado(db.promise, p, id))
      return res
        .status(409)
        .json({
          error: "Ya existe un proveedor con el mismo nombre, RFC o correo",
...
router.delete("/:id", soloAdmin, async (req, res, next) => {
  ...
  if (!Number.isInteger(id) || id <= 0)
    return res.status(400).json({ error: "Proveedor inválido" });
  if (!validarConfirmacionProveedor(req.body.confirmacion))
    return res
      .status(400)
      .json({ error: "Confirma la eliminación del proveedor" });
  const connection = await db.promise.getConnection();
  try {
    await connection.beginTransaction();
    const [[proveedor]] = await connection.query(
      "SELECT id,nombre FROM proveedores WHERE id=? FOR UPDATE",
      [id],
    );
    if (!proveedor)
      throw Object.assign(new Error("Proveedor no encontrado"), {
        status: 404,
      });
    const [[uso]] = await connection.query(
      "SELECT COUNT(*) compras FROM compras WHERE proveedor_id=?",
      [id],
    );
    if (proveedorTieneCompras(uso))
      throw Object.assign(
        new Error("No se puede eliminar: el proveedor tiene compras asociadas"),
        { status: 409 },
      );
    ...
    await connection.query(
      "DELETE FROM producto_proveedores WHERE proveedor_id=?",
      [id],
    );
