...
function proyectar(semanas) {
  const valores = semanas.map((item) =>
  ...
  const estimado =
    valores.reduce((sum, value, index) => sum + value * pesos[index], 0) / peso;
  const promedio =
    valores.reduce((sum, value) => sum + value, 0) / valores.length;
  const desviacion = Math.sqrt(
    valores.reduce((sum, value) => sum + (value - promedio) ** 2, 0) /
      valores.length,
  );
  return {
...
function mondayKey(value) {
  const date =
    value instanceof Date ? new Date(value) : new Date(`${value}T00:00:00Z`);
...
async function predictionData(database = db.promise, productId = null) {
  ...
  const [[rows], [historicalRows]] = await Promise.all([
    database.query(
      ...
      GROUP BY p.id,p.codigo,p.nombre,p.unidad,semana_inicio ORDER BY p.id,semana_inicio`,
      params,
    ),
    database.query(
      `SELECT h.producto_id id,
      ...
      GROUP BY h.producto_id,semana_inicio ORDER BY h.producto_id,semana_inicio`,
      params,
    ),
  ]);
  ...
  for (const row of rows) {
    if (!products.has(row.id))
      products.set(row.id, {
        ...
        unidad: row.unidad,
        raw: [],
        semanas_pos_disponibles: 0,
      });
    if (row.semana_inicio)
      products.get(row.id).raw.push({ ...row, fuente: "VENTAS_POS" });
  }
  ...
  for (const row of historicalRows) {
    ...
    if (!product.raw.some((item) => mondayKey(item.semana_inicio) === key))
      product.raw.push({ ...row, fuente: "INVENTARIO_HISTORICO" });
  }
  return [...products.values()].map((product) => {
    product.raw.sort((a, b) =>
      String(mondayKey(a.semana_inicio)).localeCompare(
        String(mondayKey(b.semana_inicio)),
      ),
    );
    return {
      ...product,
      semanas: completeWeeks(product.raw),
      semanas_historicas: product.raw.filter(
        (item) => item.fuente === "INVENTARIO_HISTORICO",
      ).length,
      semanas_pos: product.raw.filter((item) => item.fuente === "VENTAS_POS")
        .length,
...
router.get("/", async (req, res, next) => {
  ...
  try {
    ...
    let mlByProduct = new Map(),
      mlWarning = null;
    try {
      const ml = await runForecast(
        products.map((product) => ({
          product_id: product.producto_id,
          values: product.semanas.map((week) => week.cantidad),
        })),
      );
      mlByProduct = new Map(
        (ml.products || []).map((item) => [Number(item.product_id), item]),
      );
    } catch (error) {
    ...
    const predicciones = products.map((product) => {
      ...
      return {
        ...
        estimado_proxima_semana: Number(estimate.toFixed(2)),
        incertidumbre: Number(local.incertidumbre.toFixed(2)),
        metodo: winner,
        metodo_actual: "PROMEDIO_PONDERADO_LOCAL",
        ganador: winner,
        comparacion: comparable
          ? {
              semanas_validacion: ml.validation_weeks,
              metricas: ml.metrics,
              pronosticos: ml.forecasts,
            }
          : null,
        observacion: comparable
          ...
          : ml?.reason ||
            (local.muestra < 4
              ...
              : "No fue posible comparar con scikit-learn."),
        periodo: "PRÓXIMA_SEMANA",
        semanas_disponibles: product.semanas.length,
        fuentes: {
    ...
    return res.json({
      predicciones,
      motor_ml: mlWarning ? "NO_DISPONIBLE" : "SCIKIT_LEARN",
      advertencia_ml: mlWarning,
      criterio_ganador: "Menor MAE; RMSE se usa para desempatar.",
      limitaciones:
        "No considera clima, promociones, mermas ni eventos externos. Los ceros representan semanas sin ventas registradas.",
    });
  } catch (error) {
    return next(error);
  }
