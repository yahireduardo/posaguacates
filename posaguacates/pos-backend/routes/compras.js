...
const {
  cambiaImporteCompra,
  calcularStockEditado,
  totalCompraNoDisminuye,
} = require("../lib/edicionCompra");

const router = express.Router();
router.use(permitirRoles("ADMON_GRAL"));
const fallo = (mensaje, status = 400) =>
    ...
    const [[cabecera], [detalle]] = await Promise.all([
      db.promise.query(
        `SELECT c.*,p.nombre proveedor,u.nombre usuario,cpp.id cuenta_id,cpp.saldo_pendiente,cpp.estado estado_cuenta,
         ...
         FROM compras c LEFT JOIN proveedores p ON p.id=c.proveedor_id
         LEFT JOIN usuarios u ON u.id=c.usuario_id
         LEFT JOIN cuentas_por_pagar_proveedores cpp ON cpp.compra_id=c.id WHERE c.id=?`,
        [req.params.id],
      ),
      db.promise.query(
        `SELECT d.*,p.codigo,p.nombre FROM detalle_compra d JOIN productos p ON p.id=d.producto_id
         WHERE d.compra_id=? ORDER BY d.id`,
        [req.params.id],
      ),
    ]);
    if (!cabecera[0])
...
router.post("/", async (req, res, next) => {
  ...
  if (
    ...
    !/^[A-Za-z0-9._:-]{8,80}$/.test(key)
  ) {
    return res
      .status(400)
      .json({
        error:
          "Proveedor, productos sin repetir e Idempotency-Key válido son obligatorios",
      });
  }
  const connection = await db.promise.getConnection();
  try {
    await connection.beginTransaction();
    const [[repetida]] = await connection.query(
      "SELECT id,folio,total FROM compras WHERE idempotency_key=? FOR UPDATE",
      [key],
    );
    if (repetida) {
      ...
      return res.json({
        id: repetida.id,
        folio: repetida.folio,
        total: Number(repetida.total),
        repetida: true,
      });
    }
    const [[proveedor]] = await connection.query(
      "SELECT id FROM proveedores WHERE id=? AND activo=1 FOR UPDATE",
      [proveedorId],
    );
    ...
    for (const item of items) {
      ...
      const [[producto]] = await connection.query(
        "SELECT id,nombre,unidad,stock FROM productos WHERE id=? AND activo=1 FOR UPDATE",
        [id],
      );
      if (
        !producto ||
        String(item.unidad || producto.unidad).toUpperCase() !==
          String(producto.unidad).toUpperCase() ||
        !esCantidadValida(cantidad, producto.unidad) ||
    ...
    const folio = String(req.body.folio || `C-${Date.now()}`)
      .trim()
      .replace(/\s+/g, " ");
    const referencia =
    ...
    for (const detalle of detalles) {
      ...
      await connection.query(
        ...
        [stockFinal, detalle.costo, detalle.producto.id],
      );
      await connection.query(
    ...
    if (error.code === "ER_DUP_ENTRY")
      return res.status(409).json({ error: "Folio o solicitud duplicada" });
    next(error);
  } finally {
    connection.release();
  }
...
router.put("/:id", async (req, res, next) => {
  const id = Number(req.params.id),
    proveedorId = Number(req.body.proveedor_id);
  const items = Array.isArray(req.body.productos) ? req.body.productos : [];
  const ids = items.map((item) => Number(item.producto_id));
  if (
    !Number.isInteger(id) ||
    ...
    return res
      .status(400)
      .json({
        error: "Compra, proveedor y productos sin repetir son obligatorios",
      });
  }
  const connection = await db.promise.getConnection();
  try {
    ...
    if (compra.estado !== "ACTIVA")
      throw fallo("Solo se pueden editar compras activas", 409);
    const [[proveedor]] = await connection.query(
      "SELECT id FROM proveedores WHERE id=? AND activo=1 FOR UPDATE",
      [proveedorId],
    );
    ...
    const [[cuenta]] = await connection.query(
      "SELECT id,total_deuda,saldo_pendiente,estado FROM cuentas_por_pagar_proveedores WHERE compra_id=? FOR UPDATE",
      [id],
    );
    const [anteriores] = await connection.query(
      `SELECT dc.*,p.nombre,p.stock,p.unidad FROM detalle_compra dc JOIN productos p ON p.id=dc.producto_id
       WHERE dc.compra_id=? ORDER BY dc.producto_id FOR UPDATE`,
      [id],
    );
    const todosIds = [
      ...new Set([
        ...anteriores.map((item) => Number(item.producto_id)),
        ...ids,
      ]),
    ].sort((a, b) => a - b);
    const [productos] = await connection.query(
      `SELECT id,nombre,unidad,stock,activo FROM productos WHERE id IN (${todosIds.map(() => "?").join(",")}) ORDER BY id FOR UPDATE`,
      todosIds,
    );
    ...
    const [[pagos]] = cuenta
      ? await connection.query(
          "SELECT COUNT(*) total FROM pagos_proveedores WHERE cuenta_id=? AND estado='ACTIVO' FOR UPDATE",
          [cuenta.id],
        )
      : [[{ total: 0 }]];
    if (!totalCompraNoDisminuye(compra.total, total)) {
      throw fallo(
        `El nuevo total (${total.toFixed(2)}) no puede ser menor al total original (${Number(compra.total).toFixed(2)})`,
        409,
      );
    }
    const cambiaImporte = cambiaImporteCompra({
      compra,
      proveedorId,
      total,
      anteriores,
    ...
    if (Number(pagos.total) > 0 && cambiaImporte) {
      throw fallo(
        "La compra ya tiene pagos. Solo puedes editar folio, referencia y observaciones",
        409,
      );
    }
    const folio = String(req.body.folio || compra.folio || `C-${id}`)
      .trim()
      .replace(/\s+/g, " ");
    const referencia =
      String(req.body.referencia || "")
    ...
    if (!cambiaImporte) {
      await connection.query(
        "UPDATE compras SET folio=?,referencia=?,observaciones=? WHERE id=?",
        [folio, referencia, observaciones, id],
      );
    } else {
      const cantidadAnterior = new Map(
        anteriores.map((item) => [
          Number(item.producto_id),
          Number(item.cantidad),
        ]),
      );
      const cantidadNueva = new Map(
        nuevos.map((item) => [Number(item.producto.id), item.cantidad]),
      );
      for (const productoId of todosIds) {
        const producto = mapaProductos.get(productoId),
          anterior = cantidadAnterior.get(productoId) || 0,
          nueva = cantidadNueva.get(productoId) || 0;
        const stockFinal = calcularStockEditado(
            producto.stock,
            anterior,
            nueva,
          ),
          diferencia = Number((nueva - anterior).toFixed(2));
        if (stockFinal == null)
          throw fallo(
            `No se puede editar: el stock recibido de ${producto.nombre} ya fue consumido`,
            409,
          );
        const detalleNuevo = nuevos.find(
          (item) => Number(item.producto.id) === productoId,
        );
        await connection.query(
          "UPDATE productos SET stock=?" +
            ...
            " WHERE id=?",
          detalleNuevo
            ? [stockFinal, detalleNuevo.costo, productoId]
      ...
      await connection.query("DELETE FROM detalle_compra WHERE compra_id=?", [
        id,
      ]);
    ...
    if (error.code === "ER_DUP_ENTRY")
      return res
        .status(409)
        .json({ error: "El folio ya pertenece a otra compra" });
    next(error);
  } finally {
    connection.release();
  }
...
router.post("/:id/cancelar", async (req, res, next) => {
  ...
  if (!Number.isInteger(id) || id <= 0 || motivo.length < 5) {
    return res
      .status(400)
      .json({
        error: "Compra y motivo de al menos 5 caracteres son obligatorios",
      });
  }
  const connection = await db.promise.getConnection();
  try {
    ...
    if (!compra) throw fallo("Compra no encontrada", 404);
    if (compra.estado !== "ACTIVA")
      throw fallo("La compra ya está cancelada", 409);
    const [[cuenta]] = await connection.query(
      "SELECT id FROM cuentas_por_pagar_proveedores WHERE compra_id=? FOR UPDATE",
      [id],
    );
    if (cuenta) {
      ...
      if (Number(pagos.total) > 0)
        throw fallo(
          "No se puede cancelar una compra que ya tiene pagos a proveedor",
          409,
        );
    }
    ...
    for (const detalle of detalles) {
      ...
      if (stockFinal < 0)
        throw fallo(
          `No se puede cancelar: el stock actual de ${detalle.nombre} ya fue consumido`,
          409,
        );
    ...
    if (cuenta)
      await connection.query(
        "UPDATE cuentas_por_pagar_proveedores SET saldo_pendiente=0,estado='CANCELADA' WHERE id=?",
        [cuenta.id],
      );
    await connection.query(
      `INSERT INTO auditoria_operaciones(usuario_id,accion,entidad,entidad_id,motivo)
       VALUES (?,'CANCELAR_COMPRA','COMPRA',?,?)`,
      [req.usuario.id, String(id), motivo],
    );
    await connection.commit();
