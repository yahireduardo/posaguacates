...
function detalleValido(items) {
  if (!Array.isArray(items) || !items.length)
    throw error("La orden debe incluir productos");
  ...
  for (const item of items) {
    ...
    if (
      ...
      cantidad <= 0 ||
      (precio !== null &&
        (!Number.isInteger(precio) || precio <= 0 || precio > 99999999))
    ) {
...
async function catalogoOrden(
  ...
  const [[cliente]] = await connection.query(
    "SELECT id, nombre_razon_social FROM clientes WHERE id = ? AND activo = 1",
    [clienteId],
  );
  ...
  const [productos] = await connection.query(
    `SELECT id, codigo, nombre, precio_venta, stock, unidad, activo
     FROM productos WHERE id IN (${ids.map(() => "?").join(",")}) ORDER BY id${bloquear ? " FOR UPDATE" : ""}`,
    ids,
  );
  ...
  const detalle = productos.map((p) => {
    ...
    if (!esCantidadValida(cantidad, p.unidad))
      throw error(mensajeCantidad(p.unidad));
    const precio =
      permitirPrecio && precios.has(p.id)
        ? Number(precios.get(p.id).toFixed(2))
  ...
  return {
    cliente,
    detalle,
    total: Number(detalle.reduce((s, p) => s + p.subtotal, 0).toFixed(2)),
  };
}
...
async function listar(req, res) {
  ...
  if (estado && estados.includes(estado)) {
    where.push("ov.estado = ?");
    params.push(estado);
  }
  if (req.query.folio) {
    where.push("ov.folio LIKE ?");
    params.push(`%${String(req.query.folio).trim()}%`);
  }
  if (req.query.cliente) {
    where.push("c.nombre_razon_social LIKE ?");
    params.push(`%${String(req.query.cliente).trim()}%`);
  }
  if (req.query.fecha) {
    where.push("DATE(ov.creada_at) = ?");
    params.push(req.query.fecha);
  }
  if (numeroId(req.query.usuario_id)) {
    where.push("ov.usuario_id = ?");
  ...
  try {
    ...
    res
      ...
      .json({
        error:
          "No fue posible consultar órdenes; verifica la migración pendiente",
      });
  }
}
  ...
  try {
    const [[orden]] = await db.promise.query(
      `SELECT ov.*, c.nombre_razon_social cliente, u.nombre usuario
       FROM ordenes_venta ov JOIN clientes c ON c.id=ov.cliente_id
       JOIN usuarios u ON u.id=ov.usuario_id WHERE ov.id=?`,
      [id],
    );
    ...
    const [productos] = await db.promise.query(
      `SELECT dov.*, p.codigo, p.nombre, p.unidad, p.precio_venta precio_actual, p.stock, p.activo,
       ...
       WHERE dov.orden_id=? ORDER BY dov.id`,
      [id],
    );
    res.json({ orden, productos });
  } catch (e) {
    res.status(500).json({ error: "No fue posible consultar la orden" });
  }
});
...
async function guardar(req, res) {
  ...
  if (!clienteId || !["BORRADOR", "PENDIENTE"].includes(estado))
  ...
  try {
    ...
    if (id) {
      const [[actual]] = await connection.query(
        "SELECT estado FROM ordenes_venta WHERE id=? FOR UPDATE",
        [id],
      );
      if (!actual) throw error("Orden no encontrada", 404);
      if (!["BORRADOR", "PENDIENTE"].includes(actual.estado))
        throw error("La orden ya no puede editarse", 409);
    }
    const calculo = await catalogoOrden(
      connection,
      clienteId,
      req.body.productos,
      false,
    ...
    if (id) {
      ...
      await connection.query(
        "DELETE FROM detalle_orden_venta WHERE orden_id=?",
        [id],
      );
    } else {
      ...
      await connection.query(
        "UPDATE ordenes_venta SET folio=CONCAT('OV-',LPAD(id,8,'0')) WHERE id=?",
        [ordenId],
      );
    }
    for (const p of calculo.detalle) {
      await connection.query(
        `INSERT INTO detalle_orden_venta
         ...
         VALUES (?,?,?,?,?,NULL)`,
        [ordenId, p.id, p.cantidad, p.precio, p.subtotal],
      );
    }
    ...
    res
      .status(id ? 200 : 201)
      .json({
        mensaje: id ? "Orden actualizada" : "Orden creada",
        orden_id: ordenId,
        total_estimado: calculo.total,
      });
  } catch (e) {
    ...
    res
      .status(e.status || 500)
      .json({
        error: e.status ? e.message : "No fue posible guardar la orden",
      });
  } finally {
    connection.release();
  }
  ...
  try {
    const [result] = await db.promise.query(
      `UPDATE ordenes_venta SET estado='CANCELADA'
       WHERE id=? AND estado IN ('BORRADOR','PENDIENTE')`,
      [id],
    );
    if (!result.affectedRows)
      return res
        .status(409)
        .json({ error: "La orden no existe o ya no puede cancelarse" });
    res.json({ mensaje: "Orden cancelada" });
  } catch (e) {
    res.status(500).json({ error: "No fue posible cancelar la orden" });
  }
});
...
router.post("/:id/convertir", async (req, res) => {
  ...
  let metodoPago =
    tipoPago === "CREDITO" ? null : metodoCapturado || "EFECTIVO";
  let referenciaPago =
    tipoPago === "CREDITO"
      ? null
  ...
  try {
    ...
    const [[orden]] = await connection.query(
      "SELECT * FROM ordenes_venta WHERE id=? FOR UPDATE",
      [ordenId],
    );
    if (!orden) throw error("Orden no encontrada", 404);
    if (orden.estado !== "PENDIENTE" || orden.venta_id)
      throw error("La orden ya no está disponible", 409);
    const [[clienteCredito]] = await connection.query(
      "SELECT nombre_razon_social,permite_credito FROM clientes WHERE id=? FOR UPDATE",
      [orden.cliente_id],
    );
    if (
      tipoPago === "CREDITO" &&
      (!Number(clienteCredito?.permite_credito) ||
    ...
    const productosEntrada =
      Array.isArray(items) && items.length
        ...
        : (
            await connection.query(
              "SELECT producto_id,cantidad,precio_estimado precio_unitario FROM detalle_orden_venta WHERE orden_id=?",
              [ordenId],
            )
          )[0];
    const calculo = await catalogoOrden(
    ...
    if (tipoPago === "CONTADO") {
      desglosePago = normalizarMetodosPago(req.body, calculo.total, {
        metodo: "metodo_pago",
    ...
    for (const p of calculo.detalle)
      if (Number(p.stock) < p.cantidad)
        throw error(`Stock insuficiente para ${p.nombre}`, 409);
    ...
    if (tipoPago === "CREDITO") {
      const [cuenta] = await connection.query(
        `INSERT INTO cuentas_por_cobrar (venta_id,cliente_id,total_deuda,saldo_pendiente,estado)
         VALUES (?,?,?,?,'PENDIENTE')`,
        [venta.insertId, orden.cliente_id, calculo.total, calculo.total],
      );
      await connection.query(
    ...
    res
      .status(201)
      .json({
        mensaje: "Orden convertida",
        venta_id: venta.insertId,
        total: calculo.total,
        importe_recibido: desglosePago?.importe_recibido || calculo.total,
        cambio: desglosePago?.cambio || 0,
        productos: calculo.detalle,
      });
  } catch (e) {
    await connection.rollback();
    console.error("Error convirtiendo orden en venta:", {
      ordenId,
      code: e.code,
      message: e.message,
    });
    res
      .status(e.status || 500)
      .json({
        error: e.status ? e.message : "No fue posible convertir la orden",
      });
  } finally {
    connection.release();
  }
