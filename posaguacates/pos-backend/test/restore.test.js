...
test("compara respaldos nuevos, iguales y antiguos", () => {
  const local = {
    lastKnownSaleId: 10,
    lastOperationAt: "2026-07-30T10:00:00Z",
  };
  assert.equal(
    compararRespaldo(
      { lastKnownSaleId: 11, createdAt: "2026-07-30T11:00:00Z" },
      local,
    ).status,
    "NEWER",
  );
  assert.equal(
    compararRespaldo(
      { lastKnownSaleId: 10, createdAt: "2026-07-30T10:00:00Z" },
      local,
    ).status,
    "SAME",
  );
  const old = compararRespaldo(
    { lastKnownSaleId: 9, createdAt: "2026-07-29T10:00:00Z" },
    local,
  );
...
test("rechaza restauración sin RESTAURAR y conserva el token", async () => {
  ...
  await assert.rejects(
    service.restaurar({ token, usuarioId: 1, confirmacion: "SI" }),
    /RESTAURAR/,
  );
...
test("crea respaldo de emergencia antes del comando y ejecuta rollback ante fallo", async () => {
  ...
  const service = new RestoreService({
    ...
    audit: {
      async registrar(row) {
        order.push(`audit:${row.resultado}`);
      },
    },
    hash: async () => "correcto",
    createCredentials: async () => ({
      directorio: path.join(base, "cred"),
      ruta: path.join(base, "cred.cnf"),
    }),
  ...
  service.rollbackEmergency = async () => {
    order.push("rollback");
  };
  await assert.rejects(
    service.restaurar({ token, usuarioId: 1, confirmacion: "RESTAURAR" }),
...
test("rechaza SQL que excede el máximo y archivos incorrectos", async () => {
  ...
  await assert.rejects(
    service.analizar({ path: file, originalname: "large.sql" }, 1),
    /excede/,
  );
  await fs.rm(base, { recursive: true, force: true });
});
