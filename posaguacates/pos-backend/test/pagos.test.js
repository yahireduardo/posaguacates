...
test("rechaza un pago inexistente", () => {
  assert.throws(() => prepararReversionPago(null, [aplicacion()]), {
    status: 404,
  });
});
...
test("restaura el saldo de una aplicación sin superar la deuda", () => {
  const [resultado] = prepararReversionPago({ estado: "ACTIVO" }, [
    aplicacion(),
  ]);
...
test("prepara atómicamente todas las aplicaciones de un pago", () => {
  const resultados = prepararReversionPago({ estado: "ACTIVO" }, [
    aplicacion(),
    aplicacion({ cuenta_id: 2, saldo_pendiente: 0, monto_aplicado: 300 }),
  ]);
  assert.deepEqual(
    resultados.map((r) => r.saldo_nuevo),
    [250, 300],
  );
});
...
test("rechaza pagos ligados a una cuenta cancelada", () => {
  assert.throws(
    () =>
      prepararReversionPago({ estado: "ACTIVO" }, [
