...
test("rechaza un token inválido", () => {
  const res = respuesta();
  autenticar({ get: () => "Bearer no-es-jwt" }, res, () =>
...
test("rechaza un token expirado", () => {
  const token = jwt.sign({ id: 1, rol: "CAJERO" }, process.env.JWT_SECRET, {
    expiresIn: -1,
  });
  const res = respuesta();
  autenticar({ get: () => `Bearer ${token}` }, res, () =>
    assert.fail("no debe continuar"),
  );
  assert.equal(res.statusCode, 401);
...
test("acepta un token válido", () => {
  const token = jwt.sign({ id: 1, rol: "ADMON_GRAL" }, process.env.JWT_SECRET, {
    expiresIn: "1m",
  });
...
test("impide al cajero acceder a una operación administrativa", () => {
  const res = respuesta();
  permitirRoles("ADMON_GRAL")({ usuario: { rol: "CAJERO" } }, res, () =>
    assert.fail("no debe continuar"),
  );
