...
function dependenciasBackup(base, overrides = {}) {
  ...
  const db = {
    async query(sql) {
      ...
      if (/MAX\(id\)/.test(sql))
        return [
          [{ lastKnownSaleId: 12, lastKnownSaleAt: "2026-07-30T10:00:00Z" }],
        ];
      if (/operaciones/.test(sql))
        return [[{ lastOperationAt: "2026-07-30T10:00:00Z" }]];
  ...
  return {
    ...
    service: new BackupService({
      ...
      removeCredentials: async (credentials) => {
        if (credentials)
          await fs.rm(credentials.directorio, { recursive: true, force: true });
      },
      ...overrides,
    }),
  };
...
test("exporta ZIP con backup.sql, manifest correcto y SHA-256", async () => {
  ...
  const files = await extraerZipSeguro(
    result.zipPath,
    extract,
    10 * 1024 * 1024,
  );
  ...
  assert.match(
    await fs.readFile(files.sqlPath, "utf8"),
...
test("informa si mariadb-dump.exe no existe y audita el fallo", async () => {
  ...
  await assert.rejects(
    d.service.crearZip(1, { baseDir: base }),
...
test("propaga error de credenciales del ejecutor sin exponer contraseña", async () => {
  ...
  const d = dependenciasBackup(base, {
    runner: async () => {
      throw Object.assign(new Error("Acceso denegado"), {
...
test("crea y elimina credenciales temporales", async () => {
  ...
  assert.match(
    await fs.readFile(credentials.ruta, "utf8"),
    /password="solo-prueba"/,
  );
