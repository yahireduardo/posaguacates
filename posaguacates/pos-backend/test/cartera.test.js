...
test("aplicación automática paga primero la nota más antigua", () => {
  const r = construirAplicaciones(
    cuentas,
    { cuenta_ids: [1, 2], modo: "AUTOMATICO" },
    130,
  );
  assert.deepEqual(
    r.map((x) => [x.cuenta.id, x.monto]),
    [
      [1, 100],
      [2, 30],
    ],
    ...
    () =>
      construirAplicaciones(
        cuentas,
        {
          ...
          modo: "MANUAL",
          aplicaciones: [
            { cuenta_id: 1, monto: 50 },
            { cuenta_id: 2, monto: 20 },
          ],
        },
        80,
      ),
...
test("rechaza pago superior a los saldos elegidos", () => {
  assert.throws(
    () => construirAplicaciones(cuentas, { cuenta_ids: [1, 2] }, 181),
    /supera/,
  );
});
    ...
    () =>
      construirAplicaciones(
        cuentas,
        {
          cuenta_ids: [1],
          modo: "MANUAL",
          aplicaciones: [{ cuenta_id: 1, monto: 101 }],
        },
        101,
      ),
    /supera|inválida/,
