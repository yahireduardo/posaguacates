...
test("el modo destructivo solo se habilita con true explícito", () => {
  ...
  assert.equal(
    eliminacionDestructivaHabilitada({ ALLOW_DESTRUCTIVE_TEST_DELETES: "1" }),
    false,
  );
  assert.equal(
    eliminacionDestructivaHabilitada({
...
test("Público General está protegido por id y por nombre normalizado", () => {
  ...
  assert.equal(
    esPublicoGeneral({ id: 99, nombre_razon_social: "Público General" }),
    true,
  );
  assert.equal(
    esPublicoGeneral({ id: 3, nombre_razon_social: "Cliente prueba" }),
    false,
  );
});
...
test("la eliminación exige motivo y frase exacta", () => {
  ...
  assert.equal(
    ...
    "Escribe ELIMINAR CLIENTE para confirmar",
  );
});
