  ...
  assert.throws(
    () =>
      normalizarMetodosPago(
        { metodos_pago: [{ metodo_pago: "EFECTIVO", monto: 90 }] },
        100,
      ),
    /coincidir/,
  );
...
test("exige referencia para transferencia y cheque", () => {
  assert.throws(
    () =>
      normalizarMetodosPago(
        { metodos_pago: [{ metodo_pago: "CHEQUE", monto: 100 }] },
        100,
      ),
    /referencia/,
  );
...
test("calcula cambio cuando el efectivo recibido supera el total", () => {
  const r = normalizarMetodosPago(
    { metodos_pago: [{ metodo_pago: "EFECTIVO", monto: 600 }] },
    100,
    { permitirCambio: true },
  );
