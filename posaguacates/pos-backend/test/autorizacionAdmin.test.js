...
test("ADMON_GRAL debe volver a escribir su contraseña", async () => {
  const hash = await bcrypt.hash("Segura12345", 4);
  const r = await resolverAutorizacionAdmin({
    usuario: { id: 1, username: "admin", rol: "ADMON_GRAL" },
    body: { password_admin: "Segura12345" },
    buscarAdministrador: async () => ({ id: 1, password_hash: hash }),
  });
  assert.deepEqual(r, { solicitadoPor: 1, autorizadoPor: 1, delegada: false });
});
test("ADMON_GRAL es rechazado sin reautenticación", async () => {
  const hash = await bcrypt.hash("Segura12345", 4);
...
test("CAJERO acepta autorización bcrypt correcta y conserva su identidad", async () => {
  ...
  assert.deepEqual(r, { solicitadoPor: 2, autorizadoPor: 1, delegada: true });
});
