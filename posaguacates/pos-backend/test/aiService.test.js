...
test("recomendaciones son explicables y nunca sugieren cantidad negativa", () => {
  ...
  assert.equal(result[0].prioridad, "CRITICA");
  assert.ok(
    result.every(
      (item) =>
        item.cantidad_sugerida === undefined || item.cantidad_sugerida >= 0,
    ),
  );
  assert.ok(result.every((item) => item.evidencia));
});

test("IA calcula cobertura, riesgo y confianza del reabasto", () => {
  const analysis = inventoryAnalysis({
    stock: 2,
    stock_minimo: 5,
    venta_semanal_promedio: 8,
  });
  assert.equal(analysis.coverageWeeks, 0.25);
  assert.equal(analysis.risk, "CRITICO");
  assert.equal(analysis.confidence, "ALTA");
  assert.equal(analysis.suggested, 18);
  const inventory = buildRecommendations(context)[0];
  assert.equal(inventory.prioridad, "CRITICA");
  assert.equal(inventory.evidencia.semanas_cobertura, 0.25);
});
...
test("IA local responde según la intención de la pregunta", () => {
  ...
  assert.match(
    buildLocalAnswer(context, "¿Qué tendencia está bajando?"),
    /Criollo/,
  );
});
...
test("asistente generativo usa Responses API sin almacenar la petición", async () => {
  ...
  const result = await generateAnswer({
    ...
    fetchImpl: async (url, options) => {
      ...
      return {
        ok: true,
        json: async () => ({ id: "resp_1", output_text: "Todo en orden." }),
      };
    },
  });
