...
test("rechaza nombre vacío, teléfono, correo y RFC inválidos", () => {
  ...
  assert.match(
    validarProveedor(datosProveedor({ nombre: "Proveedor", telefono: "abc" })),
    /teléfono/i,
  );
  assert.match(
    validarProveedor(
      datosProveedor({ nombre: "Proveedor", correo: "correo@" }),
    ),
    /correo/i,
  );
  assert.match(
    validarProveedor(datosProveedor({ nombre: "Proveedor", rfc: "INVALIDO" })),
    /RFC/,
  );
});
