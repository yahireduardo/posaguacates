...
function construirAplicaciones(cuentas, body, monto) {
  ...
  const totalSaldos = elegibles.reduce(
    (s, c) => s + Number(c.saldo_pendiente),
    0,
  );
  if (monto > totalSaldos + 0.001)
    throw fallo("El pago supera los saldos seleccionados", 409);
  if (String(body.modo || "AUTOMATICO").toUpperCase() === "MANUAL") {
    const mapa = new Map(
      (body.aplicaciones || []).map((a) => [
        Number(a.cuenta_id),
        Number(a.monto),
      ]),
    );
    const aplicaciones = elegibles.map((c) => ({
      cuenta: c,
      monto: mapa.get(Number(c.id)) || 0,
    }));
    if (
      aplicaciones.some(
        (a) => a.monto <= 0 || a.monto > Number(a.cuenta.saldo_pendiente),
      )
    )
      throw fallo("Distribución manual inválida");
    if (
      Math.abs(aplicaciones.reduce((s, a) => s + a.monto, 0) - monto) > 0.005
    ) {
