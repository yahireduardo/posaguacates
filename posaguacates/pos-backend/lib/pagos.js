...
function prepararReversionPago(pago, aplicaciones) {
  ...
  if (pago.estado === "CANCELADO")
    throw fallo("El pago ya está cancelado", 409);
  if (!Array.isArray(aplicaciones) || !aplicaciones.length) {
    throw fallo("El pago no tiene aplicaciones para revertir", 409);
  }
  if (
    aplicaciones.some((aplicacion) => aplicacion.cuenta_estado === "CANCELADA")
  ) {
    throw fallo(
      "No se puede revertir un pago ligado a una venta cancelada",
      409,
    );
  }
