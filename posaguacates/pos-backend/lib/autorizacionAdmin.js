...
async function resolverAutorizacionAdmin({
  usuario,
  body,
  buscarAdministrador,
}) {
  ...
  if (usuario.rol === "ADMON_GRAL") {
    const administrador = await buscarAdministrador(
    ...
    if (!autorizado)
      throw errorHttp(
        "Se requiere volver a escribir la contraseña administrativa",
        401,
      );
    return {
      solicitadoPor: Number(usuario.id),
      autorizadoPor: Number(usuario.id),
      delegada: false,
    };
  }
...
async function registrarAuditoriaSiExiste(connection, datos) {
  try {
    ...
    if (error.code === "ER_NO_SUCH_TABLE") {
      console.warn(
        "Auditoría pendiente: aplique sql/migracion_autorizaciones_admin.sql",
      );
      return false;
    }
    throw error;
