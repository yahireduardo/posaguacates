    ...
    try {
      ...
      if (estado.restauracion_en_progreso) {
        return res
          .status(503)
          .json({
            error: "Restauración en progreso. Intenta nuevamente al finalizar.",
          });
      }
      if (estado.estado === "ENTREGADA" || Number(estado.bloqueada) === 1) {
        return res.status(423).json({
          error:
            "Esta computadora entregó la base de datos. Restaure el respaldo más reciente para continuar.",
        });
      }
