...
class BackupService {
  ...
  async metadataLocal() {
    const [[venta]] = await this.db.query(
      "SELECT MAX(id) lastKnownSaleId, MAX(fecha) lastKnownSaleAt FROM ventas",
    );
  ...
  async crearZip(usuarioId, opciones = {}) {
    ...
    try {
      ...
      const manifest = {
        ...
        sqlSize: stat.size,
        sha256,
        lastKnownSaleId: local.lastKnownSaleId,
        lastKnownSaleAt: local.lastKnownSaleAt,
        lastOperationAt: local.lastOperationAt,
        generatedByUserId: Number(usuarioId),
      };
      ...
      try {
        await this.audit.registrar({
          ...
          resultado: "FALLIDO",
          error:
            error.stderr || error.sqlMessage || error.message || error.code,
        });
      } catch (_) {
      ...
      if (error.code === "ENOENT") {
        throw Object.assign(
          new Error("No se encontró la herramienta de respaldo de MariaDB."),
          { status: 503 },
        );
      }
      throw error;
