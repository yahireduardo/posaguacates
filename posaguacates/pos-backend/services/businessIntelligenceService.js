...
async function getBusinessContext(database = db.promise) {
  const [[summaryRows], [inventoryRows], [trendRows]] = await Promise.all([
    database.query(`SELECT COUNT(*) ventas_30d, COALESCE(SUM(total),0) ingresos_30d,
      ...
      (SELECT COUNT(*) FROM ventas WHERE estado_venta='ACTIVA' AND fecha>=DATE_SUB(CURDATE(),INTERVAL 60 DAY) AND fecha<DATE_SUB(CURDATE(),INTERVAL 30 DAY)) ventas_30d_anteriores,
      (SELECT COALESCE(SUM(total),0) FROM ventas WHERE estado_venta='ACTIVA' AND fecha>=DATE_SUB(CURDATE(),INTERVAL 60 DAY) AND fecha<DATE_SUB(CURDATE(),INTERVAL 30 DAY)) ingresos_30d_anteriores,
  ...
  return {
    ...
    inventario: inventoryRows,
    tendencias: trendRows,
