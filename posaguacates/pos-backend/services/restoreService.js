...
const {
  crearCredencialesTemporales,
  eliminarCredencialesTemporales,
} = require("../utils/tempCredentials");
...
function compararRespaldo(manifest, local) {
  if (!manifest)
    return {
      status: "UNKNOWN",
      label: ESTADOS.UNKNOWN,
      warnings: ["El SQL no incluye manifiesto verificable."],
    };
  const backupSale = Number(manifest.lastKnownSaleId || 0);
  const localSale = Number(local.lastKnownSaleId || 0);
  const backupDate = Date.parse(
    manifest.lastOperationAt || manifest.createdAt || "",
  );
  const localDate = Date.parse(local.lastOperationAt || "");
  let status = "UNKNOWN";
  if (
    backupSale > localSale ||
    (backupSale === localSale && backupDate > localDate)
  )
    status = "NEWER";
  else if (
    backupSale === localSale &&
    (!Number.isFinite(backupDate) ||
      !Number.isFinite(localDate) ||
      backupDate === localDate)
  )
    status = "SAME";
  else if (
    backupSale < localSale ||
...
class AnalysisTokens {
  ...
  get(token, usuarioId) {
    ...
    if (
      !datos ||
      datos.expiresAt < this.now() ||
      Number(datos.usuarioId) !== Number(usuarioId)
    ) {
      ...
      throw Object.assign(
        new Error("El análisis venció o no corresponde a esta sesión"),
        { status: 400 },
      );
    }
    return datos;
...
class RestoreService {
  constructor({
    ...
    audit,
    env = process.env,
    runner = ejecutarProceso,
    tokens = new AnalysisTokens({
      ttlMs: Number(process.env.BACKUP_ANALYSIS_TOKEN_TTL_MS || 600000),
    }),
    hash = hashFile,
    unzip = extraerZipSeguro,
    createCredentials = crearCredencialesTemporales,
  ...
  maxBytes() {
    return Number(this.env.BACKUP_MAX_SIZE_MB || 500) * 1024 * 1024;
  }

  async analizar(upload, usuarioId) {
    if (!upload?.path)
      throw Object.assign(new Error("Seleccione un archivo de respaldo"), {
        status: 400,
      });
    const extension = path.extname(upload.originalname || "").toLowerCase();
    if (![".zip", ".sql"].includes(extension))
      throw Object.assign(new Error("Solo se aceptan respaldos ZIP o SQL"), {
        status: 400,
      });
    ...
    if (
      ...
      throw Object.assign(
        new Error(
          "El respaldo no es un archivo válido o excede el tamaño permitido",
        ),
        { status: 400 },
      );
    }
    ...
    try {
      if (extension === ".zip") {
        const extraido = await this.unzip(
          upload.path,
          workingDir,
          this.maxBytes(),
        );
        ...
        if (
          manifest.format !== "POS_AGUACATES_BACKUP" ||
          manifest.version !== 1 ||
          manifest.database !== String(this.env.DB_NAME || "posaguacates") ||
          manifest.sqlFile !== "backup.sql"
        ) {
          throw Object.assign(
            new Error("El manifiesto no corresponde a un respaldo compatible"),
            { status: 400 },
          );
        }
      } else {
      ...
      if (
        manifest &&
        (Number(manifest.sqlSize) !== sqlStat.size ||
          manifest.sha256 !== sha256)
      ) {
        throw Object.assign(
          new Error("El respaldo está dañado: tamaño o hash incorrecto"),
          { status: 400 },
        );
      }
      ...
      if (!/POS_AGUACATES_BACKUP|MariaDB dump|MySQL dump/i.test(cabecera)) {
        throw Object.assign(
          new Error(
            "El archivo seleccionado no corresponde a un respaldo SQL válido",
          ),
          { status: 400 },
        );
      }
      ...
      if (manifest?.backupId) {
        ...
        if (Number(restaurado.total) > 0)
          comparison.warnings.push(
            "Este backup_id ya fue restaurado anteriormente.",
          );
        await this.db.query(
      ...
      await this.audit.registrar({
        ...
        nombreArchivo: upload.originalname,
        resultado: "EXITOSO",
  ...
  async restaurar({
    ...
    if (confirmacion !== "RESTAURAR")
      throw Object.assign(new Error("Escriba RESTAURAR para confirmar"), {
        status: 400,
      });
    ...
    if (analisis.comparison.status === "OLDER" && confirmarAntiguo !== true) {
      throw Object.assign(
        new Error(
          "Confirme expresamente la restauración de un respaldo más antiguo",
        ),
        { status: 400 },
      );
    }
    const actualHash = await this.hash(analisis.sqlPath);
    if (actualHash !== analisis.sha256)
      throw Object.assign(new Error("El archivo cambió después del análisis"), {
        status: 400,
      });
    ...
    try {
      ...
      emergency = await this.backupService.crearZip(usuarioId, {
        emergency: true,
      });
      const preRestoreDir = path.join(
        path.resolve(this.env.BACKUP_DIR || "C:\\posaguacates\\backups"),
        "pre-restore",
      );
      await fs.mkdir(preRestoreDir, { recursive: true });
      const emergencyZip = path.join(
        preRestoreDir,
        `PRE_RESTORE_${Date.now()}.zip`,
      );
      await fs.copyFile(emergency.zipPath, emergencyZip);
      await this.aplicarRetencion(
        preRestoreDir,
        Number(this.env.PRE_RESTORE_RETENTION_COUNT || 10),
      );
      const dbConfig = configDb(this.env);
      credentials = await this.createCredentials(dbConfig);
      const executable = path.join(
        path.resolve(
          this.env.MARIADB_BIN_DIR || "C:\\Program Files\\MariaDB 12.3\\bin",
        ),
        "mariadb.exe",
      );
      ...
      await this.runner(
        executable,
        [
          `--defaults-extra-file=${credentials.ruta}`,
          "--ssl=0",
          dbConfig.database,
        ],
        {
      ...
      await this.instanceControl.activarTrasRestauracion(
        analisis.manifest?.backupId || null,
      );
      await this.db.query(
        `INSERT INTO historial_traslados
        ...
        [
          analisis.manifest?.backupId || crypto.randomUUID(),
          "backup.sql",
          analisis.manifest?.hostname || null,
          os.hostname(),
          usuarioId,
          analisis.manifest?.lastKnownSaleId || null,
          "Reinicio del servicio requerido",
        ],
      );
      await this.audit.registrar({
        usuarioId,
        accion: "RESTAURAR",
        backupId: analisis.manifest?.backupId,
        nombreArchivo: "backup.sql",
        resultado: "EXITOSO",
      });
      return {
        ok: true,
        restartRequired: true,
        emergencyBackup: path.basename(emergencyZip),
      };
    } catch (error) {
      ...
      try {
        await this.audit.registrar({
          ...
          backupId: analisis.manifest?.backupId,
          nombreArchivo: "backup.sql",
          resultado: recuperada ? "RECUPERADO" : "FALLIDO",
          error: error.message,
        });
      } catch (_) {}
      throw Object.assign(
        new Error(
          recuperada
            ...
            : "La restauración falló. Se requiere recuperación manual.",
        ),
      ...
      if (emergency?.tempDir)
        await fs.rm(emergency.tempDir, { recursive: true, force: true });
  ...
  async rollbackEmergency(emergency, credentials) {
    ...
    try {
      const { sqlPath } = await this.unzip(
        emergency.zipPath,
        temp,
        this.maxBytes(),
      );
  ...
  async aplicarRetencion(directorio, conservar) {
    ...
    for (const entrada of entradas) {
      if (!entrada.isFile() || !/^PRE_RESTORE_.*\.zip$/i.test(entrada.name))
        continue;
