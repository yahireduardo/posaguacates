...
function runForecast(products, options = {}) {
  ...
  const timeoutMs = Math.min(
    120000,
    Math.max(3000, Number(env.ML_TIMEOUT_MS) || 30000),
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      finish(
        Object.assign(
          new Error("El análisis con scikit-learn excedió el tiempo permitido"),
          { code: "ML_TIMEOUT" },
        ),
      );
    }, timeoutMs);
    ...
    child.on("error", (error) =>
      finish(
        Object.assign(
          new Error(`Python no está disponible: ${error.message}`),
          { code: "ML_UNAVAILABLE" },
        ),
      ),
    );
    child.on("close", (code) => {
      if (code !== 0)
        return finish(
          Object.assign(
            new Error(
              `El modelo no pudo ejecutarse${stderr ? `: ${stderr.trim()}` : ""}`,
            ),
            { code: "ML_FAILED" },
          ),
        );
      try {
        ...
        return finish(
          Object.assign(
            new Error("El modelo devolvió una respuesta inválida"),
