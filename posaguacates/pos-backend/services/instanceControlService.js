...
class InstanceControlService {
  ...
  async obtenerInstanceId() {
    ...
    if (configurado) {
      if (!/^[0-9a-f-]{36}$/i.test(configurado))
        throw new Error("INSTANCE_ID no tiene formato UUID");
    ...
    try {
      ...
      if (!/^[0-9a-f-]{36}$/i.test(creadoPorOtraPeticion))
        throw new Error("El INSTANCE_ID local no es válido");
  ...
  async marcarEntregada(backupId, motivo = "Base entregada para traslado") {
    ...
    if (Number(respaldo.total) !== 1) {
      throw Object.assign(
        new Error(
          "El respaldo no fue generado por esta computadora o ya fue entregado",
        ),
        { status: 409 },
      );
    }
    ...
    if (!result.affectedRows)
      throw Object.assign(new Error("No se encontró la instalación local"), {
        status: 409,
      });
    await this.db.query(
  ...
  async iniciarRestauracion({ timeoutMs = 30000 } = {}) {
    if (this.restaurando)
      throw Object.assign(new Error("Ya existe una restauración en progreso"), {
        status: 409,
      });
    ...
    if (this.escriturasActivas > 0) {
      this.restaurando = false;
      throw Object.assign(
        new Error("Hay operaciones en curso. Intente restaurar nuevamente."),
        { status: 409 },
      );
    }
  }
