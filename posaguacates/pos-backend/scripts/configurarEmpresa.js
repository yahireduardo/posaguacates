...
async function main() {
  ...
  const host = await pedir('Servidor MariaDB', '127.0.0.1');
  const port = Number(await pedir('Puerto MariaDB', '3306'));
  const adminDb = await pedir('Usuario administrador de MariaDB', 'root');
  const adminDbPassword = await preguntarOculto('Contraseña del administrador de MariaDB: ');
  const database = await pedir('Nombre de la base', 'posaguacates');
  const appUser = await pedir('Usuario exclusivo del POS', 'pos_app');
  const appPassword = await preguntarOculto('Nueva contraseña para pos_app: ');
  ...
  try {
    ...
    if (Number(users.total) === 0) {
      const nombre = await pedir('Nombre del primer administrador', 'Administrador');
      const username = await pedir('Usuario para iniciar sesión', 'admin');
      const password = await preguntarOculto('Contraseña del primer administrador: '); validarPassword(password);
      const confirm = await preguntarOculto('Confirma la contraseña: '); if (password !== confirm) throw new Error('Las contraseñas no coinciden');
