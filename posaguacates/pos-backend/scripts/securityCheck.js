...
const forbiddenPath = (file) => {
  ...
  if (/(?:^|\/)node_modules(?:\/|$)/.test(normalized))
    return "node_modules versionado";
  if (/(?:^|\/)\.env(?:\.|$)/.test(normalized) && !allowedEnv.test(normalized))
    return ".env real versionado";
  if (/\.(?:zip|bak|backup|dump|sql\.gz|log)$/i.test(normalized))
    return "respaldo o log versionado";
  if (
    /(?:^|\/)(?:backups|logs?|data|temp|tmp|restore|restoration)(?:\/|$)/i.test(
      normalized,
    )
  )
    return "artefacto local versionado";
  if (/(?:^|\/)(?:instance-id|\.instance-id|\.maintenance)$/i.test(normalized))
    return "estado local versionado";
  if (/\.(?:pem|p12|pfx|key|credentials|cnf)$/i.test(normalized))
    return "credencial o clave versionada";
  return null;
};
...
for (const file of tracked) {
  ...
  if (
    /\bAIza[0-9A-Za-z_-]{30,}\b|\bgh[opusr]_[0-9A-Za-z]{30,}\b|\beyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\b/.test(
      content,
    )
  ) {
  ...
  if (allowedEnv.test(file)) {
    for (const key of ["DB_PASSWORD", "JWT_SECRET", "GEMINI_API_KEY"]) {
      ...
      const safe =
        !value ||
        /(?:change|replace|example|test|fictici|generate|generar|opcional|vac[ií]o|your|<|>)/i.test(
          value,
        );
  ...
  if (
    /\.sql$/i.test(file) &&
    !/sql\/migrations\//i.test(file) &&
    /^\s*(?:INSERT|REPLACE|LOAD\s+DATA)\b/im.test(content)
  ) {
    failures.push({
      file,
      reason: "SQL no migratorio contiene datos insertados",
    });
  }
}
...
console.log(
  `Security check correcto: ${tracked.length} archivos versionados revisados sin secretos ni artefactos prohibidos.`,
);
