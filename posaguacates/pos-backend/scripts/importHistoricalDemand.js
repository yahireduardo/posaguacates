...
function extract(files, env = process.env) {
  ...
  const script = path.join(
    ...
    "..",
    "ml",
    "extract_historical_inventory.py",
  );
  return new Promise((resolve, reject) => {
    const child = spawn(executable, [script, ...files], {
      windowsHide: true,
    ...
    child.on("close", (code) => {
      if (code !== 0)
        return reject(
          new Error(errorOutput.trim() || `Python terminó con código ${code}`),
        );
      try {
        return resolve(JSON.parse(output));
      } catch {
        return reject(new Error("El extractor devolvió JSON inválido"));
      }
    });
  });
}
...
function sourceKey(record) {
  return crypto
    .createHash("sha256")
    .update(
      ["INVENTARIO_XLSX", record.file, record.sheet, record.date, "01"].join(
        "|",
      ),
    )
    .digest("hex");
...
async function importHistoricalDemand(files, database = db.promise) {
  if (!files.length)
    throw new Error("Indique al menos un archivo de inventario .xlsx");
  const extracted = await extract(files);
  const [[product]] = await database.query(
    "SELECT id FROM productos WHERE codigo='01' AND activo=1 LIMIT 1",
  );
  if (!product) throw new Error("No existe el producto activo con código 01");
  let imported = 0;
  for (const record of extracted.records) {
    await database.query(
      `INSERT INTO demanda_historica
  ...
  return {
    imported,
    futureRemoved: removed.affectedRows,
...
if (require.main === module) {
  importHistoricalDemand(process.argv.slice(2))
    .then((result) =>
      console.log(
        `Importación terminada: ${result.imported} lotes. Futuros excluidos: ${result.futureRemoved}. Advertencias: ${result.warnings.length}`,
      ),
    )
    .catch((error) => {
