...
async function preflight(connection) {
  ...
  if (database.nombre !== name)
    throw new Error("La conexión no corresponde a DB_NAME");
  ...
  if (tables.length !== 4)
    throw new Error("Faltan tablas base; no se aplicaron migraciones");
  if (tables.some((table) => table.motor !== "InnoDB"))
    throw new Error("Las tablas base deben usar InnoDB");
}
