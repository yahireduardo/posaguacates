...
async function obtenerSidWindows(ejecutar = execFileAsync) {
  ...
  if (!sid)
    throw new Error("No se pudo identificar el SID de la cuenta del servicio");
  return sid;
}
...
async function crearCredencialesTemporales(config, opciones = {}) {
  ...
  try {
    ...
    const entornoPruebaAislado =
      process.env.NODE_ENV === "test" &&
      process.env.TEST_DATABASE === "true" &&
      /_test$/i.test(process.env.DB_NAME || "");
    if (
      process.platform === "win32" &&
      opciones.protegerWindows !== false &&
      !entornoPruebaAislado
    ) {
...
async function eliminarCredencialesTemporales(credenciales, opciones = {}) {
  ...
  try {
    await rm(credenciales.directorio, {
      recursive: true,
      force: true,
      maxRetries: 8,
      retryDelay: 150,
    ...
    try {
      await (opciones.chmod || fs.chmod)(credenciales.ruta, 0o600);
    } catch (_) {}
