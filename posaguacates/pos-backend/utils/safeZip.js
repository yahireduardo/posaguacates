...
function validarNombre(nombre) {
  ...
  if (
    !PERMITIDOS.has(normalizado) ||
    path.posix.basename(normalizado) !== normalizado
  ) {
    throw Object.assign(
      new Error("El ZIP contiene archivos o rutas no permitidos"),
      { status: 400 },
    );
  }
}
...
async function extraerZipSeguro(zipPath, destino, maxBytes) {
  ...
  try {
    for await (const entry of zip.eachEntry()) {
      ...
      if (encontrados.has(entry.fileName))
        throw Object.assign(new Error("El ZIP contiene archivos duplicados"), {
          status: 400,
        });
      const tipoUnix = (entry.externalFileAttributes >>> 16) & 0o170000;
      if (tipoUnix === 0o120000)
        throw Object.assign(new Error("No se permiten enlaces en el ZIP"), {
          status: 400,
        });
      total += entry.uncompressedSize;
      if (total > maxBytes)
        throw Object.assign(
          new Error("El respaldo excede el tamaño permitido"),
          { status: 413 },
        );
  ...
  if (
    !encontrados.has("backup.sql") ||
    !encontrados.has("manifest.json") ||
    encontrados.size !== 2
  ) {
    throw Object.assign(
      new Error("El ZIP debe contener únicamente backup.sql y manifest.json"),
      { status: 400 },
    );
  }
  return {
