...
if (!process.env.JWT_SECRET) {
  console.error(
    "Falta JWT_SECRET. Copia .env.example a .env y define una clave segura.",
  );
  process.exit(1);
}
...
app.use((req, res, next) => {
  ...
  if (
    req.method !== "GET" &&
    req.method !== "HEAD" &&
    fs.existsSync(mantenimiento)
  ) {
    return res
      .status(503)
      .json({
        error: "Sistema temporalmente en mantenimiento por restauración",
      });
  }
  return next();
});
...
function crearLimitadorLogin({
  ...
  return (req, res, next) => {
    ...
    const key = req.ip;
    const recientes = (intentosFallidos.get(key) || []).filter(
      (time) => now() - time < ventanaMs,
    );
    ...
    res.once("finish", () => {
      if (res.statusCode === 401)
        intentosFallidos.set(key, [...recientes, now()]);
...
app.use(
  "/cuentas-proveedores",
  ...sesion,
  require("./routes/cuentasProveedores"),
);
...
async function iniciarServidor() {
  ...
  console.log(
    `Iniciando POS Aguacates. Esperando MySQL en ${process.env.DB_HOST || "localhost"}:${process.env.DB_PORT || 3306}...`,
  );
